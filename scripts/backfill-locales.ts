...
function buildFileContent(originalSource: string, spec: LocaleSpec, entries: Record<string, string>): string {
  // Preserve everything up to the `export const ... = {` line
  ...
  const header = originalSource.slice(0, headerEnd);
...
async function processLocale(spec: LocaleSpec, enEntries: Record<string, string>) {
  ...
  const existing = await loadLocale(spec.file, spec.exportName);
