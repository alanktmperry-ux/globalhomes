export interface FaqItem {
  id: string;
  question: string;
  answer: string;
  category: 'general' | 'agents' | 'buyers' | 'renters' | 'vendors' | 'auctions' | 'billing' | 'technical' | 'property-managers' | 'halo';
  tags: string[];
}

export const FAQ_ITEMS: FaqItem[] = [
  // GENERAL
  {
    id: 'what-is-listhq',
    question: 'What is ListHQ?',
    answer: 'ListHQ is an Australian real estate marketplace connecting buyers, renters, vendors, and real estate agents. Agents can list residential and rental properties, manage open homes and auctions, and communicate with clients — all in one platform.',
    category: 'general',
    tags: ['about', 'platform', 'overview'],
  },
  {
    id: 'who-can-use',
    question: 'Who can use ListHQ?',
    answer: 'ListHQ is designed for four types of users:\n\n• Real estate agents and property managers — to list and manage properties\n• Home buyers — to search, save, and enquire on properties for sale\n• Renters — to find and apply for rental properties\n• Vendors (home sellers) — to track their property\'s performance and receive reports from their agent',
    category: 'general',
    tags: ['users', 'roles'],
  },
  {
    id: 'is-it-free',
    question: 'Is ListHQ free to use?',
    answer: 'Searching for and enquiring on properties is completely free for buyers and renters. Real estate agents pay a subscription fee to list properties and access the full suite of agent tools. Visit our pricing page or contact us for current agent subscription rates.',
    category: 'general',
    tags: ['pricing', 'free', 'cost'],
  },
  {
    id: 'which-states',
    question: 'Which states does ListHQ cover?',
    answer: 'ListHQ covers all Australian states and territories — New South Wales, Victoria, Queensland, South Australia, Western Australia, Tasmania, the Australian Capital Territory, and the Northern Territory.',
    category: 'general',
    tags: ['coverage', 'states', 'australia'],
  },
  {
    id: 'create-account',
    question: 'How do I create an account?',
    answer: 'Click "Sign Up" in the top navigation. You can register with your email address or sign in with Google. Once registered, select your role — buyer, renter, or agent. Agents will be prompted to complete profile verification before publishing listings.',
    category: 'general',
    tags: ['account', 'signup', 'register'],
  },
  {
    id: 'reset-password',
    question: 'How do I reset my password?',
    answer: 'On the login page, click "Forgot password?" and enter your email address. You\'ll receive a reset link within a few minutes. Check your spam folder if it doesn\'t arrive. The reset link expires after 24 hours.',
    category: 'general',
    tags: ['password', 'login', 'account'],
  },
  {
    id: 'general-mobile-app',
    question: 'Is there a mobile app for ListHQ?',
    answer: 'ListHQ is available as a Progressive Web App (PWA) — you can install it directly from your browser without going to the App Store or Google Play.\n\nTo install on iPhone/iPad: open ListHQ in Safari, tap the Share button, then tap "Add to Home Screen".\n\nTo install on Android: open ListHQ in Chrome, tap the three-dot menu, then tap "Add to Home Screen" or "Install App".\n\nOnce installed, ListHQ works like a native app — it opens full screen, works offline for recently viewed content, and can be found on your home screen. This works for both agents and buyers. A dedicated App Store app is on the roadmap.',
    category: 'general',
    tags: ['mobile', 'app', 'PWA', 'install', 'iPhone', 'Android', 'home screen'],
  },

  // AGENTS
  {
    id: 'agent-create-listing',
    question: 'How do I create a property listing?',
    answer: 'From your agent dashboard, click "New Listing" or go to Dashboard → My Listings → Add Listing. Fill in the property details, upload photos, set the price, and choose the listing type (sale, rental, off-market, EOI, or auction). You can save as a draft before publishing.',
    category: 'agents',
    tags: ['listing', 'create', 'publish'],
  },
  {
    id: 'agent-listing-types',
    question: 'What types of listings can I create?',
    answer: 'ListHQ supports five listing types:\n\n• For Sale — standard residential sale with price\n• For Rent — rental property with weekly rent and availability date\n• Auction — links to the full auction management system\n• Off-Market — private listings visible only to registered subscribers\n• Expression of Interest (EOI) — no advertised price, buyers submit written offers by a set date',
    category: 'agents',
    tags: ['listing types', 'sale', 'rent', 'auction', 'off-market', 'eoi'],
  },
  {
    id: 'agent-photos',
    question: 'How do I upload and reorder photos?',
    answer: 'In the listing editor, go to the Photos tab. Drag and drop images or click to upload. You can upload JPEG, PNG, or WEBP files up to 10MB each. Drag photos to reorder them — the first photo is used as the hero image on the listing page and in search results.',
    category: 'agents',
    tags: ['photos', 'upload', 'images'],
  },
  {
    id: 'agent-open-home',
    question: 'How do I schedule open homes?',
    answer: 'From the listing detail page, go to the "Open Homes" tab and click "Schedule Open Home". Set the date, start and end time, and maximum capacity. Visitors can register online. You\'ll receive automated reminders to registered attendees 24 hours and 1 hour before the inspection. Waitlisting is supported if capacity is reached.',
    category: 'agents',
    tags: ['open home', 'inspection', 'schedule'],
  },
  {
    id: 'agent-auction-setup',
    question: 'How do I set up and run an auction?',
    answer: 'From the listing detail page, go to the "Auction" tab and click "Setup Auction". Enter the auction date, time, location (or mark as online), auctioneer details, and reserve price (confidential — never shown to buyers). On auction day, click "Start Live Auction" to open the Live Auction Console where you can record bids in real time, declare vendor bids, and conclude with SOLD or Passed In. Registered bidders and the public can watch the live bid feed.',
    category: 'agents',
    tags: ['auction', 'live', 'reserve', 'vendor bid'],
  },
  {
    id: 'agent-auction-registration',
    question: 'How do buyers register to bid at auction?',
    answer: 'Buyers click "Register to Bid" on the property page. They complete a 3-step form: personal details, photo ID verification, and confirmation. You review each registration in the Auction tab under "Registrations" and approve them individually. Approved bidders receive a confirmation email with their paddle number. You must verify their photo ID before granting approval.',
    category: 'agents',
    tags: ['auction', 'registration', 'paddle', 'bidder'],
  },
  {
    id: 'agent-crm',
    question: 'How does the CRM work?',
    answer: 'The CRM is in your dashboard under "Leads". Every enquiry, open home registration, and saved-property interaction creates a lead automatically. You can add notes, set follow-up reminders, tag leads (hot/warm/cold), and track their journey. Leads can be linked to specific listings. The CRM shows last contact date, enquiry source, and lead score.',
    category: 'agents',
    tags: ['crm', 'leads', 'enquiries', 'follow up'],
  },
  {
    id: 'agent-profile',
    question: 'How do I set up my public agent profile?',
    answer: 'Go to Dashboard → Profile. Add your headshot, banner image, headline, bio, and real estate licence number. Your profile page is at listhq.com.au/agent/[your-slug] and shows your active and sold listings, reviews, and average rating. Toggle "Make profile public" to make it discoverable in agent search.',
    category: 'agents',
    tags: ['profile', 'public', 'bio', 'reviews'],
  },
  {
    id: 'agent-reviews',
    question: 'How do agent reviews work?',
    answer: 'Reviews are submitted by clients via your agent profile page. ListHQ sends the reviewer a verification email — the review only publishes once they click the confirmation link. This prevents fake reviews. Reviews show a 1–5 star rating and written comment. You cannot delete reviews, but you can report a review to ListHQ support if it violates our guidelines.',
    category: 'agents',
    tags: ['reviews', 'ratings', 'verification'],
  },
  {
    id: 'agent-vendor-dashboard',
    question: 'How does the vendor performance report work?',
    answer: 'Each listing has a "Performance" tab showing your vendor a real-time dashboard: total views, enquiries, saves, open home attendance, traffic sources, and device breakdown. You can generate a shareable vendor report link that gives your vendor read-only access to their property\'s stats — no login required. Reports can also be sent via email.',
    category: 'agents',
    tags: ['vendor', 'performance', 'report', 'analytics'],
  },
  {
    id: 'agent-documents',
    question: 'How do I share documents with buyers and vendors?',
    answer: 'Use the Document Vault on each listing (Dashboard → My Listings → [Property] → Documents). Upload files like the Contract of Sale, Section 32, building inspection reports, and floor plans. You control who can access each document — agent only, vendor, verified buyers, or public. Buyers can request access to restricted documents. All downloads are logged.',
    category: 'agents',
    tags: ['documents', 'contract', 'section 32', 'vault'],
  },
  {
    id: 'agent-saved-searches',
    question: 'How does the Saved Search Alerts system work?',
    answer: 'Buyers who save a search on ListHQ are automatically notified when a new listing matches their criteria. As an agent, when you publish a new listing, it is automatically matched against all saved searches and alert emails are sent to matching buyers within minutes. You can also see how many buyers have matching saved searches for a suburb before you list.',
    category: 'agents',
    tags: ['saved search', 'alerts', 'notifications', 'buyers'],
  },
  {
    id: 'agent-off-market',
    question: 'How do off-market listings work?',
    answer: 'Off-market listings are visible only to buyers who have subscribed to off-market alerts for that suburb. Go to Dashboard → Off-Market to manage subscriber lists. When you publish an off-market listing, subscribers matching the criteria (price range, property type, suburb) receive an email notification. The listing does not appear in public search results.',
    category: 'agents',
    tags: ['off-market', 'private', 'subscribers'],
  },
  {
    id: 'agent-rental-pm',
    question: 'How do I manage rental listings and applications?',
    answer: 'Create a listing and select "For Rent" as the listing type. Set the weekly rent, bond amount, available date, and lease terms. Renters can submit applications directly through ListHQ. Applications are reviewed in the PM Dashboard under "Applications", where you can shortlist, approve, or decline applicants and request additional documents.',
    category: 'agents',
    tags: ['rental', 'property management', 'applications', 'pm'],
  },
  {
    id: 'agent-billing',
    question: 'How does agent billing work?',
    answer: 'Agent subscriptions are managed from Dashboard → Billing. You can view your current plan, see your subscription status, and update your details. Payment processing via Stripe is launching soon — founding agents will be notified by email when billing goes live. Until then, your access continues uninterrupted.',
    category: 'agents',
    tags: ['billing', 'subscription', 'stripe', 'payment'],
  },
  {
    id: 'agent-multilingual-translation',
    question: 'How does multilingual listing translation work?',
    answer: 'ListHQ automatically translates your listing into every language using AI: Mandarin (Simplified), Mandarin (Traditional), Vietnamese, Korean, Arabic, and Japanese. When you publish a listing, translation is queued automatically. Once complete, a language toggle appears on the property detail page — buyers can switch languages with one click.\n\nEach translation includes:\n• Full title and description in the target language\n• A 1–2 sentence cultural summary\n• Cultural highlights specific to that buyer community (e.g. school proximity, feng shui orientation, garden space) — only where genuinely applicable\n• Agent insights on multicultural buyer appeal\n\nTranslations are generated automatically — you do not need to do anything after publishing. You can manually trigger a re-translation from the listing detail page if you update the description.',
    category: 'agents',
    tags: ['multilingual', 'translation', 'mandarin', 'vietnamese', 'korean', 'arabic', 'japanese', 'language', 'AI', 'chinese'],
  },
  {
    id: 'agent-language-toggle',
    question: 'How do buyers switch the listing language, and what does the agent see?',
    answer: 'On any property detail page, buyers use the language selector in the navigation bar to switch between English, 中文 Simplified, 中文 Traditional, Tiếng Việt, 한국어, العربية, and 日本語. The entire listing — title, description, cultural highlights, and agent insights — switches language instantly without a page reload.\n\nAs the listing agent, you will see an "Agent Insights" panel alongside each translation. This panel summarises the property\'s multicultural appeal, suggested buyer profiles (e.g. "Korean-speaking families seeking school catchments"), and key selling points for diverse buyer communities. This panel is visible to you but not to buyers.',
    category: 'agents',
    tags: ['language', 'toggle', 'multilingual', 'buyer', 'translation', 'agent insights', 'korean', 'arabic', 'japanese'],
  },
  {
    id: 'agent-voice-listing',
    question: 'How does voice listing creation work?',
    answer: 'In the listing creation flow, Step 4 (Voice) lets you describe the property out loud instead of typing. Click the microphone button and speak naturally — describe the property\'s features, style, and standout qualities. Your speech is transcribed in real time and displayed as editable text.\n\nFrom there, click "Generate with AI" and choose a tone:\n• Standard — clear and professional\n• Luxury — aspirational, premium language\n• Family — warm, lifestyle-focused\n• Investment — yield and capital growth focused\n\nThe AI rewrites your transcript into a polished listing description in your chosen tone. You can edit the result before saving. This typically takes under 30 seconds.',
    category: 'agents',
    tags: ['voice', 'listing', 'AI', 'description', 'microphone', 'generate', 'tone'],
  },
  {
    id: 'agent-cobroke',
    question: 'What is the co-broke toggle and how does it work?',
    answer: 'The co-broke toggle is in Step 5 (Settings) of the listing creation flow. When enabled, it signals to other ListHQ agents that you are open to co-agency — meaning another agent can introduce a buyer and share the commission.\n\nWhen co-broke is enabled:\n• Other agents can see the listing is co-broke eligible in their dashboard\n• You can set an auto-decline threshold — any co-broke offer below a specified commission percentage is automatically declined\n• Incoming co-broke requests appear in your CRM as leads, tagged as "Co-broke"\n\nThis is particularly useful for off-market listings where you want maximum buyer reach without going public. Co-broke arrangements are between agents — the buyer is not shown any commission information.',
    category: 'agents',
    tags: ['co-broke', 'cobroke', 'co-agency', 'commission', 'off-market', 'settings'],
  },
  {
    id: 'agent-team-invite',
    question: 'How do I invite a team member to my agency?',
    answer: 'Go to Dashboard → Team. You can add team members in two ways:\n\n1. Invite by Email — Click "Invite by Email", enter their email address, select their role (Agent or Admin), and set their access level (Full Access or Read Only). They will receive an email with a link to join your agency. If they already have a ListHQ account, they are added immediately.\n\n2. Invite Code — Click "Create Code" to generate a reusable 8-character code. Share the code with your team member — they enter it at Dashboard → Team → Join with Code. You can set a maximum number of uses and deactivate the code at any time.\n\nRoles available:\n• Agent — standard access to listings, CRM, and calendar\n• Admin — can manage listings, team members, and settings\n• Principal — full control, including billing and agency branding (set during agency creation)\n\nYour plan determines how many seats are available. Each team member gets their own login.',
    category: 'agents',
    tags: ['team', 'invite', 'agency', 'member', 'code', 'email', 'role', 'seat', 'agent', 'admin'],
  },
  {
    id: 'agent-team-roles',
    question: 'What is the difference between Agent, Admin, and Principal roles?',
    answer: 'ListHQ uses three team roles:\n\n• Agent — can create and manage their own listings, view their own CRM contacts and leads, schedule open homes, and access their own analytics. Cannot see other agents\' contacts or change agency settings.\n\n• Admin — all Agent permissions, plus the ability to manage all team members\' listings, view agency-wide analytics, manage invite codes, and edit agency details. Cannot change billing.\n\n• Principal — full control over everything, including billing, subscription management, team member deactivation, contact and listing reassignment, and compliance tracking. Only one Principal per agency (set during onboarding). The Principal can reassign contacts or listings from one agent to another and deactivate agents while preserving their data.',
    category: 'agents',
    tags: ['role', 'principal', 'admin', 'agent', 'team', 'permissions', 'access'],
  },
  {
    id: 'agent-team-access-levels',
    question: 'What is the difference between Full Access and Read Only for team members?',
    answer: 'When inviting a team member, you set their access level in addition to their role:\n\n• Full Access — the team member can create, edit, publish, and delete listings, add and manage CRM contacts, and take all actions within their role permissions.\n\n• Read Only — the team member can view listings, contacts, analytics, and reports, but cannot create, edit, or delete anything. Useful for support staff, business development managers, or compliance reviewers who need visibility without edit rights.',
    category: 'agents',
    tags: ['access', 'read only', 'full access', 'team', 'permissions'],
  },
  {
    id: 'agent-cma',
    question: 'How do I run a Comparative Market Analysis (CMA)?',
    answer: 'The CMA tool is available in your agent dashboard under Market → CMA. It allows you to produce a professional market analysis report to support a listing presentation or vendor appraisal.\n\nTo run a CMA:\n1. Enter the subject property address\n2. ListHQ pulls comparable sold and listed properties from the same suburb, filtered by property type, bedroom count, and sale method\n3. Review and select which comparables to include or exclude\n4. The CMA report calculates a suggested price range based on median and average comparable sale prices, adjusted for days on market\n5. Export the report as a PDF to share with your vendor\n\nThe CMA report includes a cover page, comparable property table, price per square metre analysis, and a recommended listing price range. It is designed to be presented directly to vendors at the appraisal meeting.',
    category: 'agents',
    tags: ['CMA', 'comparative market analysis', 'appraisal', 'valuation', 'vendor', 'report', 'market'],
  },
  {
    id: 'agent-suburb-intelligence',
    question: 'How does the suburb intelligence tool work?',
    answer: 'The Suburb Intelligence panel (Dashboard → Market → Suburb Intelligence) gives you a live market snapshot for any suburb you manage or prospect in.\n\nWhat it shows:\n• Median sale price and median days on market for the past 90 days\n• Active listing count vs. 90-day average — indicates whether supply is rising or falling\n• Auction clearance rate for the suburb\n• Buyer demand index — derived from saved searches and enquiry volume in the suburb\n• Price per square metre trend over the last 12 months\n\nHow to use it:\nType any suburb into the search box and the panel updates instantly. You can use this data to support your vendor appraisal conversations — showing a vendor real-time demand signals for their suburb helps justify your recommended price range and sale strategy.\n\nYou can also compare up to three suburbs side-by-side using the "Compare suburbs" button.',
    category: 'agents',
    tags: ['suburb intelligence', 'market data', 'median price', 'days on market', 'auction clearance', 'appraisal'],
  },
  {
    id: 'agent-lead-marketplace',
    question: 'What is the Lead Marketplace?',
    answer: 'The Lead Marketplace (Dashboard → Lead Marketplace) is a curated feed of buyer and renter leads that are available for agents to claim. These leads come from buyers who have submitted enquiries or voice searches on ListHQ but have not yet been connected to a specific agent.\n\nEach lead shows:\n• Buyer type (purchaser, investor, renter)\n• Property requirements (suburb, price range, bedrooms, property type)\n• Urgency (immediate, 1–3 months, flexible)\n• Lead source (voice search, open home, enquiry)\n\nYou can claim a lead to receive the buyer\'s full contact details and add them to your CRM. Claimed leads are removed from the marketplace so other agents cannot claim the same buyer. Lead Marketplace access is included on Agency and Professional plans.',
    category: 'agents',
    tags: ['lead marketplace', 'leads', 'buyers', 'claim', 'enquiry', 'marketplace'],
  },
  {
    id: 'agent-vendor-report-link',
    question: 'How do I generate a shareable vendor performance report link?',
    answer: 'On any listing detail page in your dashboard, go to the Performance tab. Click "Generate Vendor Link" to create a unique, read-only URL for your vendor.\n\nThe vendor link gives your vendor access to a live performance dashboard showing:\n• Total listing views and unique visitors\n• Number of enquiries received\n• Open home attendance numbers\n• Traffic sources (search, direct, social media, email)\n• How the listing compares to similar properties in the suburb\n\nThe link requires no login — your vendor can open it on any device at any time. The data updates in real time. You can regenerate the link at any time, which invalidates the previous one. Links can also be sent directly from the dashboard via email.',
    category: 'agents',
    tags: ['vendor', 'report', 'link', 'performance', 'analytics', 'shareable', 'views'],
  },
  {
    id: 'agent-onboarding-complete',
    question: 'How do I complete the agency onboarding setup?',
    answer: 'When you first sign up as an agent, you will be prompted to complete a 5-step agency setup (Dashboard → Agency Setup):\n\n1. Choose your path — Starting fresh (new agency, $0 trust balance) or Migrating from another system (PropertyMe, Console Cloud, Reapit, or TrustSoft)\n2. Agency & Licence Details — enter your agency or trading name, ABN, real estate licence number, principal name, state of operation, phone, address, and email\n3. Trust Account — enter your trust account bank details (bank name, BSB, account number)\n4. Cut-over date (migration path only) — the date from which all trust transactions will be recorded in ListHQ\n5. Import opening balance (migration path only) — use the Trust Import Wizard to bring in existing ledger data\n\nOnce complete, you will be taken to your full agent dashboard. You can update your agency details at any time from Dashboard → Team → Edit Details.',
    category: 'agents',
    tags: ['onboarding', 'setup', 'agency', 'trust account', 'ABN', 'licence', 'getting started'],
  },
  {
    id: 'agent-buyer-concierge',
    question: 'What is the Buyer Concierge and how do I use it?',
    answer: 'The Buyer Concierge is your agent dashboard for AI-matched buyers. It shows you which buyers are actively searching for properties that match your listings — ranked by how closely their search intent matches what you have listed.\n\nAccess it from Dashboard → Buyer Concierge. You will see:\n• Hot Buyers — the top 10 buyers scored for readiness and intent match\n• Matches by Listing — buyers grouped against each of your active listings\n• All Matches — a searchable table of every matched buyer\n\nFor each buyer you can see their search intent (e.g. "3 bedroom house near schools under $900k in Templestowe"), readiness score, and activity history. Click any buyer to open a contact modal and send them a message via anonymous relay — protecting their privacy until they choose to respond.',
    category: 'agents',
    tags: ['buyer concierge', 'buyer matching', 'AI', 'leads', 'intent', 'dashboard'],
  },
  {
    id: 'agent-buyer-readiness-score',
    question: 'What is the buyer readiness score?',
    answer: 'The buyer readiness score is a 0–100 score that estimates how close a buyer is to making a purchase decision, based on their behaviour on ListHQ.\n\nThe score is calculated from:\n• Number of searches conducted (up to 30 points)\n• Mortgage calculator use (+15 points)\n• Stamp duty calculator use (+10 points)\n• Properties saved to shortlist (up to 24 points)\n• High search frequency — more than 5 searches in 14 days (+20 points)\n• Narrowing searches to a specific suburb (+15 points)\n• Revisiting the same listing multiple times (+10 points)\n\nScore bands:\n• 71–100 — High readiness (shown in green) — actively searching, likely ready within weeks\n• 41–70 — Medium readiness (shown in amber) — engaged but still early\n• 0–40 — Low readiness (muted) — browsing, not yet committed\n\nScores update automatically as buyers use the platform. No buyer personally identifiable information is exposed — you see intent and behaviour, not names or contact details, unless the buyer chooses to respond.',
    category: 'agents',
    tags: ['buyer readiness', 'score', 'lead quality', 'buyer intent', 'matching'],
  },
  {
    id: 'agent-inspection-reports',
    question: 'How do I conduct a property inspection and condition report?',
    answer: 'Go to Dashboard → Rent Roll, find the tenancy, and click "Inspections". From there:\n\n1. Click "Schedule Inspection" and select the type — Entry (move-in), Routine (periodic), or Exit (move-out). Enter the inspection date, owner name and email. The system checks your state\'s minimum notice period and warns you if the date is too soon.\n\n2. On the day, open the inspection from the Rent Roll. Work through each room on your phone — tap the condition (Excellent / Good / Fair / Poor / Damaged), add notes, and upload photos directly from your camera. Photos are saved to the property\'s secure storage and appear inline when the report is viewed or printed.\n\n3. Record the water meter reading, key count, and smoke alarm test results in the overview section.\n\n4. Flag any maintenance issues directly from the room — set priority (Urgent / Normal / Low) and they appear in a maintenance summary at the bottom.\n\n5. Add any rooms the default list does not include. Tap the "Add Room" card, choose from quick-add chips (Pool, Study/Office, Rumpus Room, Sunroom, Balcony, Granny Flat, Storage Room, Gym) or type any custom room name. Custom rooms work the same as standard ones.\n\n6. When complete, tap "Finalise Report". A dialog lets you choose:\n • Send to Owner + Tenant — emails both at the same time\n • Send to Owner only\n • Send to Tenant only\n • Finalise without sending — saves the report without emailing anyone\n\nTenants receive a link to view the report online and can formally Accept it or raise Concerns within their state\'s dispute window. All responses are recorded against the inspection.\n\nFor Exit inspections, a "Routine History" panel automatically appears above the rooms list, showing every completed routine inspection for that tenancy with room-by-room conditions and notes. This gives you a side-by-side comparison of how each room\'s condition has changed over the tenancy — essential for fair bond claim assessment.\n\nOnce finalised, a Download PDF button appears in the top right corner of the report. The PDF includes all rooms, conditions, photos (by count), maintenance items, and key/meter/smoke alarm data, with page numbers and the property address in the footer.',
    category: 'agents',
    tags: ['inspection', 'condition report', 'entry report', 'exit report', 'routine inspection', 'property management', 'tenant', 'landlord', 'pdf', 'custom rooms', 'photos', 'routine history'],
  },
  {
    id: 'agent-seller-score',
    question: 'What is the Seller Likelihood Score and how do I use it?',
    answer: 'The Seller Likelihood Score (Dashboard → Seller Scoring) ranks your off-market and inactive listings by how likely that vendor is to be ready to sell now.\n\nEach property is scored out of 100 across three factors:\n• Time on your books — properties held longer score higher\n• Listing views — high interest with no sale suggests motivation\n• Days since last update — stale listings indicate the vendor may be reconsidering\n\nScores are colour-coded:\n• Green (70+) — Hot: proactively reach out now\n• Amber (40–69) — Warm: good time for a check-in\n• Grey (below 40) — Low: monitor for now\n\nClick any property to open the Draft Outreach panel, which pre-fills a suggested email or call script using your name, agency, and phone number. Use this as your starting point for the outreach conversation.',
    category: 'agents',
    tags: ['seller score', 'seller likelihood', 'off-market', 'outreach', 'vendor', 'scoring'],
  },
  {
    id: 'agent-network-sharing',
    question: 'How does the Agent Network off-market sharing work?',
    answer: 'The Agent Network (Dashboard → Network) lets you share your off-market and pocket listings with other ListHQ agents, and view listings shared by other agents in your market.\n\nTo share a listing:\n1. Go to the "My Listings" tab in the Network section\n2. Find the listing and click "Share to Network"\n3. Set the referral split percentage — the commission share you are offering the introducing agent\n4. The listing appears in the network feed for other agents\n\nTo claim a network listing:\n1. Browse the "Network Listings" tab\n2. Click "Contact Agent" to introduce your buyer and open a message thread\n3. If a deal proceeds, the referral split you agreed on applies\n\nYou can also post Buyer Briefs — requirements from your active buyers (price range, suburb, property type, urgency). Other agents can match their listings to your briefs and contact you directly.',
    category: 'agents',
    tags: ['network', 'off-market', 'sharing', 'referral', 'co-broke', 'buyer brief', 'pocket listing'],
  },
  {
    id: 'agent-arrears-chase',
    question: 'How do I chase rent arrears and notify tenants?',
    answer: 'The Rent Roll automatically calculates arrears for each tenancy based on the rent amount, frequency, and payment records.\n\nIn the Rent Roll table, tenancies with overdue rent show an orange "Arrears" badge with the number of days overdue and estimated amount owed.\n\nTo chase arrears:\n• Click the "Chase" button on the arrears badge to send an automated arrears notice directly to the tenant\'s email. The email states the property address, amount overdue, and your contact details.\n• For lease expiry reminders, click "Notify" on any tenancy approaching its end date to send the tenant a formal renewal notice.\n\nAll notices are sent from ListHQ\'s email system and logged against the tenancy record. For formal legal notices (breach notices, Notice to Vacate), use your state tenancy authority\'s prescribed forms — ListHQ notices are a courtesy first step.',
    category: 'agents',
    tags: ['arrears', 'rent overdue', 'chase', 'tenant notice', 'property management', 'lease expiry'],
  },
  {
    id: 'agent-inspection-schedule',
    question: 'How does routine inspection scheduling work?',
    answer: 'From your Rent Roll (Dashboard → Rent Roll), click the "Inspections" button on any tenancy row. The system automatically suggests an Australian-standard inspection schedule based on the lease start date:\n\n• Entry Inspection — on the move-in date\n• Routine at 3 months — first check after the tenant moves in\n• Routine every 6 months thereafter — ongoing throughout the tenancy\n• Exit Inspection — on the lease end date (optional, can be added when needed)\n\nAll dates are fully editable before saving. You can also check or uncheck individual inspections to schedule only what you need. Once you click "Save Inspections", the selected inspections are created and the tenant receives a formal inspection notice email for each scheduled date.\n\n**Calendar view**\nThe Inspections dashboard (Dashboard → Inspections) includes a Calendar tab (between Upcoming and Overdue). It shows a full monthly calendar with colour-coded pills for every scheduled inspection: blue for Entry, teal for Routine, orange for Exit. Click any pill to open that inspection report. Use the arrow buttons to navigate between months.\n\n**Auto-schedule annual routines**\nThe "Auto-Schedule Routines" button (top right of the Inspections page) bulk-plans routine inspections across your entire portfolio at once. The system analyses each active tenancy, works out how many routines it still needs to reach 4 for the year, and proposes dates approximately 91 days apart from the last scheduled inspection, automatically enforcing each state\'s minimum notice period. A preview shows the planned dates per property before anything is saved. Confirm to create all inspections in one action and send tenant notices.',
    category: 'agents',
    tags: ['inspection', 'routine inspection', 'schedule', 'property management', 'tenant notice', 'calendar view', 'auto-schedule', 'bulk schedule'],
  },
  {
    id: 'agent-inspection-notice',
    question: 'What notice do I need to give a tenant before a routine inspection?',
    answer: 'Minimum notice requirements for routine inspections vary by state under Australian tenancy law:\n\n• NSW — minimum 7 days notice, maximum 4 inspections per year\n• VIC — minimum 48 hours notice, maximum 1 inspection per 6 months\n• QLD — minimum 24 hours notice, maximum 4 inspections per year\n• WA — minimum 7 days notice, maximum 4 inspections per year\n• SA — minimum 7 days notice, maximum 4 inspections per year\n• TAS — minimum 24 hours notice\n• ACT — minimum 7 days notice, maximum 4 inspections per year\n• NT — minimum 24 hours notice\n\nWhen you schedule an inspection in ListHQ, a formal inspection notice email is automatically sent to the tenant\'s email address on record. The email includes the property address, inspection type, scheduled date, and your name as property manager. Always schedule inspections far enough in advance to meet your state\'s minimum notice requirements.',
    category: 'agents',
    tags: ['inspection notice', 'tenant', 'notice period', 'routine inspection', 'state law', 'NSW', 'VIC', 'QLD'],
  },
  {
    id: 'agent-inspection-frequency',
    question: 'How many routine inspections can I conduct per year?',
    answer: 'Australian tenancy law limits the frequency of routine inspections to protect tenant privacy. The limits vary by state:\n\n• NSW — maximum 4 per year (every 3 months)\n• VIC — maximum 1 per 6-month period (most restrictive in Australia)\n• QLD — maximum 4 per year\n• WA — maximum 4 per year\n• SA — maximum 4 per year\n• ACT — maximum 4 per year\n• TAS and NT — no legislated maximum, but inspections must be reasonable\n\nListHQ\'s suggested inspection schedule follows best practice for all states — entry at move-in, a 3-month check, then 6-monthly thereafter. This complies with all state limits and is the industry standard for professional property managers. You can adjust the schedule to suit your state\'s specific requirements.',
    category: 'agents',
    tags: ['inspection frequency', 'routine inspection', 'per year', 'state limits', 'property management'],
  },
  {
    id: 'agent-inspection-pdf',
    question: 'How do I download a PDF copy of an inspection report?',
    answer: 'Once an inspection has been finalised, open it from Dashboard → Inspections or via the Rent Roll. A "Download PDF" button appears in the top right corner of the report (it only shows once the report is read-only — i.e. after finalising).\n\nThe PDF includes:\n• Property address, inspection type (Entry / Routine / Exit), and conducted date\n• Water meter reading, key count, and smoke alarm test results\n• Every room with its condition rating, notes, number of photos, and any maintenance items flagged\n• Overall inspection notes\n• Page N of N footer with the property address on every page\n\nThe file downloads as "inspection-report-[report-id].pdf" and can be emailed, stored, or printed. You can also send the report to the owner and tenant directly when finalising — the PDF download is available separately for your own records.',
    category: 'agents',
    tags: ['inspection', 'pdf', 'download', 'report', 'condition report', 'export'],
  },
  {
    id: 'agent-inspection-calendar',
    question: 'How does the inspection calendar view work?',
    answer: 'The Inspections dashboard (Dashboard → Inspections) has a Calendar tab, positioned between the Upcoming and Overdue tabs. Click it to switch from the list view to a full monthly calendar.\n\nThe calendar shows:\n• A standard month grid starting on Monday\n• Colour-coded pills for each inspection: blue for Entry, teal for Routine, orange for Exit\n• Today\'s date highlighted with a filled circle\n• Days from the previous or next month shown in a muted colour for context\n\nInteractions:\n• Click any pill to open that inspection report directly\n• If a day has more inspections than fit in the cell, a "+N more" label appears — click it to see the full list\n• Use the left and right arrow buttons to move between months\n• A colour legend at the bottom of the calendar reminds you what each colour represents\n\nCancelled inspections are not shown in the calendar.',
    category: 'agents',
    tags: ['inspection', 'calendar', 'calendar view', 'schedule', 'monthly view', 'property management'],
  },
  {
    id: 'agent-inspection-auto-schedule',
    question: 'How do I auto-schedule annual routine inspections for all my properties?',
    answer: 'The Auto-Schedule Routines feature lets you bulk-plan routine inspections across your entire portfolio in one action.\n\nHow to use it:\n1. Go to Dashboard → Inspections\n2. Click the "Auto-Schedule Routines" button in the top right\n3. The system analyses every active tenancy and calculates:\n   • How many routines have already been scheduled or completed this year\n   • How many more are needed to reach 4 routines per year (the Australian standard)\n   • The best dates — approximately 91 days apart from the last inspection\n   • The earliest legally allowed date, enforcing your state\'s minimum notice period as a floor\n4. A preview modal shows each tenancy, the planned dates, and a badge showing how many inspections will be added\n5. Review the plan. If any dates look wrong you can adjust them manually before confirming.\n6. Click "Confirm & Schedule All" to create all inspections at once\n\nProperties that already have 4 or more routines scheduled for the year are skipped automatically. Tenant notice emails are sent for every newly created inspection.\n\nThis is the fastest way to bring your entire rent roll up to the legally required inspection cadence at the start of each year.',
    category: 'agents',
    tags: ['inspection', 'auto-schedule', 'routine inspection', 'bulk', 'annual', 'schedule all', 'property management'],
  },
  {
    id: 'agent-rent-arrears',
    question: 'How do I chase rent arrears from the Rent Roll?',
    answer: 'When a tenant is in arrears (their rent payment period has expired), the Rent Roll table shows an arrears badge in red with the number of days overdue and the estimated amount owed.\n\nTo send a formal arrears notice:\n1. Locate the tenancy in the Rent Roll table\n2. Click the "Chase" button that appears when the tenant is overdue\n3. A pre-drafted arrears notice email is sent to the tenant\'s email address with the amount owed and a request to bring the account up to date\n\nFor tenants significantly overdue (14+ days), the arrears badge turns red. Australian tenancy law specifies the required notice periods before you can issue a Notice to Vacate for non-payment of rent — consult your state\'s residential tenancy authority for breach and termination procedures.',
    category: 'agents',
    tags: ['arrears', 'rent arrears', 'overdue', 'chase', 'notice', 'property management'],
  },
  {
    id: 'agent-lease-expiry',
    question: 'How do I send a lease expiry reminder to a tenant?',
    answer: 'Leases expiring within 60 days are flagged with an "Expiring soon" badge in the Rent Roll table. A "Notify" button appears next to those tenancies.\n\nClick "Notify" to send the tenant an automated email reminder that their lease is expiring, including the lease end date and how many days remain. The email prompts the tenant to contact you to discuss renewal options.\n\nFor lease renewals, you will need to issue a new lease agreement. This is done outside of ListHQ — consult your state\'s standard lease form or use your agency\'s documentation system.',
    category: 'agents',
    tags: ['lease expiry', 'renewal', 'notify tenant', 'expiring', 'rent roll'],
  },

  // BUYERS
  {
    id: 'buyer-search',
    question: 'How do I search for properties?',
    answer: 'Use the search bar on the homepage or the Search page. You can type a suburb, postcode, or address, or use the voice search button. Filter by price range, property type, bedrooms, bathrooms, land size, and features. Use the map view to browse by location. Toggle "Sold" to see recent sold results in a suburb.',
    category: 'buyers',
    tags: ['search', 'filter', 'map'],
  },
  {
    id: 'buyer-suburb-research',
    question: 'How do I research a suburb before buying?',
    answer: 'ListHQ provides several suburb research tools on every listing page and suburb profile:\n\n• Recently Sold — scroll to "Recently Sold in [Suburb]" to see comparable sales with sold prices, days on market, and price per square metre.\n• School Catchments — the Schools section shows which primary and secondary schools include the property in their catchment zone.\n• Suburb Profile — click the suburb name on any listing to see median prices, days on market, auction clearance rates, and demographic data for that area.\n• Saved Search Alerts — set up alerts for a suburb so you are notified the moment a new listing matches your criteria. This helps you track how fast properties are moving.\n\nFor sold price history across a whole suburb, search for the suburb and select the "Sold" tab to see all recent transactions.',
    category: 'buyers',
    tags: ['suburb', 'research', 'sold prices', 'schools', 'demographics', 'market data'],
  },
  {
    id: 'buyer-save-property',
    question: 'How do I save a property to my shortlist?',
    answer: 'Click the heart icon on any listing card or property page to save it to your shortlist. Sign in or create a free account to save across devices. View your saved properties under your profile → Shortlist. You\'ll receive a notification if the price changes or the property is sold.',
    category: 'buyers',
    tags: ['save', 'shortlist', 'favourite', 'heart'],
  },
  {
    id: 'buyer-saved-search',
    question: 'How do I set up property alerts?',
    answer: 'Run a search with your preferred filters (suburb, price, bedrooms etc.) then click "Save Search" at the top of the results. Give your search a name and choose how often you want alerts: instant, daily digest, or weekly. You\'ll receive an email whenever a new listing matches your saved criteria.',
    category: 'buyers',
    tags: ['alerts', 'saved search', 'notifications', 'email'],
  },
  {
    id: 'buyer-open-home',
    question: 'How do I register for an open home?',
    answer: 'On the property page, find the "Open Homes" section and click "Register" next to your preferred time. Enter your name, email, and phone number. You\'ll receive a confirmation email and automatic reminders 24 hours and 1 hour before the inspection. If a time is full, you can join the waitlist.',
    category: 'buyers',
    tags: ['open home', 'inspection', 'register'],
  },
  {
    id: 'buyer-enquire',
    question: 'How do I contact the listing agent?',
    answer: 'On any listing page, click "Enquire" or "Send Message" to contact the agent directly. You can also call or email the agent using the contact details shown. All messages are logged in your account so you have a record of conversations.',
    category: 'buyers',
    tags: ['enquire', 'contact', 'agent', 'message'],
  },
  {
    id: 'buyer-auction-register',
    question: 'How do I register to bid at an auction?',
    answer: 'On the property page, click "Register to Bid". You\'ll complete a 3-step registration: personal details (name, phone, email), photo ID details (driver\'s licence, passport, or Medicare card), and a confirmation checklist. The agent reviews and approves your registration. Once approved, you\'ll receive a confirmation with your paddle number.\n\nImportant: The cooling-off period does NOT apply to properties purchased at auction in most Australian states. Make sure your finance and building inspections are complete before bidding.',
    category: 'buyers',
    tags: ['auction', 'register', 'bidding', 'paddle'],
  },
  {
    id: 'buyer-pre-auction-offer',
    question: 'Can I make an offer before the auction?',
    answer: 'Yes. On the property page, expand the "Submit a Pre-Auction Offer" section. Enter your offer amount, deposit details, settlement terms, and any conditions (subject to finance, subject to building inspection). The agent will present your offer to the vendor. Note: the vendor is not obligated to accept pre-auction offers, and cooling-off does NOT apply if the offer is accepted prior to the scheduled auction date.',
    category: 'buyers',
    tags: ['pre-auction', 'offer', 'before auction'],
  },
  {
    id: 'buyer-mortgage-calc',
    question: 'How does the mortgage calculator work?',
    answer: 'The mortgage calculator is available on every listing page and at the Mortgage Calculator page. Enter the purchase price, deposit amount, loan term, and interest rate. The calculator applies the APRA serviceability buffer (+3%) to show you the assessed rate used by banks. It also provides a full amortisation schedule and repayment breakdown. Stamp duty estimates are included based on the property\'s state.',
    category: 'buyers',
    tags: ['mortgage', 'calculator', 'repayments', 'stamp duty', 'apra'],
  },
  {
    id: 'buyer-what-sold',
    question: 'How do I find out what properties have sold for nearby?',
    answer: 'On any property page, scroll down to the "Recently Sold in [Suburb]" section to see comparable sales. For a full list of all recent sales in a suburb, click "View all sales in [suburb]" or search for the suburb and select the "Sold" tab. Sales data includes the sold price, days on market, sale method, and price per square metre.',
    category: 'buyers',
    tags: ['sold', 'comparable sales', 'recent sales', 'suburb'],
  },
  {
    id: 'buyer-school-catchment',
    question: 'How do I check school catchment zones?',
    answer: 'On any property listing page, scroll down to the "Schools" section. It shows the primary and secondary schools whose catchment zones include the property address, along with distance and school ratings. You can also filter properties by school catchment in the search filters.',
    category: 'buyers',
    tags: ['schools', 'catchment', 'zones', 'education'],
  },
  {
    id: 'buyer-documents',
    question: 'How do I access property documents like the Contract of Sale?',
    answer: 'On the property page, scroll to the "Documents" section. Public documents (like floor plans) are immediately downloadable. Restricted documents (like the Contract of Sale or Section 32) require you to be a registered and verified buyer. Click "Request Access" to ask the agent to grant you access. You\'ll receive an email once access is approved.',
    category: 'buyers',
    tags: ['documents', 'contract', 'section 32', 'download'],
  },

  {
    id: 'buyer-voice-search',
    question: 'How does voice search work?',
    answer: 'Voice search lets you describe what you are looking for out loud instead of typing filters manually.\n\nOn the homepage, click the microphone button in the search bar. Speak naturally — for example: "Three bedroom house in Bondi under a million dollars" or "Pet-friendly apartment near the city for rent under $600 a week".\n\nListHQ transcribes your speech and uses AI to extract the key details — suburb, price range, bedrooms, property type, and whether you are buying or renting. You are then taken directly to the matching search results with all filters pre-applied.\n\nVoice search works best in a quiet environment. Make sure your browser has microphone permission enabled for listhq.com.au.',
    category: 'buyers',
    tags: ['voice search', 'microphone', 'speak', 'AI', 'search'],
  },

  // RENTERS
  {
    id: 'renter-search',
    question: 'How do I search for rental properties?',
    answer: 'On the homepage or Search page, select "Rent" from the listing type tabs. Filter by suburb or postcode, weekly rent range, property type, bedrooms, and features like pets allowed or parking. You can also search by available date to find properties ready when you need them.',
    category: 'renters',
    tags: ['search', 'rental', 'rent', 'filter'],
  },
  {
    id: 'renter-apply',
    question: 'How do I apply for a rental property?',
    answer: 'On the rental listing page, click "Apply Now". The 4-step application wizard asks for: personal details and identification, rental history (previous addresses and references), employment and income details, and supporting documents (payslips, bank statements, ID). Completed applications are submitted directly to the property manager for review.',
    category: 'renters',
    tags: ['apply', 'application', 'rental', 'tenancy'],
  },
  {
    id: 'renter-application-status',
    question: 'How do I track my rental application?',
    answer: 'Log in and go to your profile → My Applications. You\'ll see the status of each application: Submitted, Under Review, Shortlisted, Approved, or Declined. You\'ll also receive email notifications when your status changes. If you\'re approved, the property manager will contact you to arrange lease signing and bond payment.',
    category: 'renters',
    tags: ['application', 'status', 'tracking', 'approved'],
  },
  {
    id: 'renter-documents',
    question: 'Once I\'m approved, how do I access tenancy documents?',
    answer: 'Once your application is approved, you\'ll gain access to the Document Vault for that property. Documents available to approved tenants include the lease agreement, condition report, and property information statements. Go to your application → Documents, or visit the property page and sign in to access the Documents section.',
    category: 'renters',
    tags: ['documents', 'lease', 'condition report', 'tenant'],
  },
  {
    id: 'renter-pets',
    question: 'Can I filter for pet-friendly properties?',
    answer: 'Yes. In the search filters, check "Pets considered" under the Features section. Note that "pets considered" means the landlord is open to discussion — it is not a guarantee. Always confirm pet arrangements with the property manager before submitting an application.',
    category: 'renters',
    tags: ['pets', 'filter', 'rental'],
  },
  {
    id: 'renter-bond',
    question: 'How much bond will I need to pay?',
    answer: 'Bond amounts are set by the landlord and displayed on each listing. In most Australian states, residential bond is capped at 4 weeks\' rent. Bond is lodged with the relevant state authority (e.g., RTBA in Victoria, Rental Bonds Board in NSW) and is returned at the end of the tenancy subject to the property\'s condition.',
    category: 'renters',
    tags: ['bond', 'deposit', 'rental'],
  },

  // VENDORS
  {
    id: 'vendor-report',
    question: 'How do I view my property\'s performance report?',
    answer: 'Your listing agent will send you a Vendor Report link by email. The link gives you read-only access to your property\'s real-time performance dashboard — no login required. You\'ll see total views, enquiries, open home attendance, where buyers are finding your listing (search, direct, social), and how your property compares to similar listings in the suburb.',
    category: 'vendors',
    tags: ['vendor', 'report', 'performance', 'analytics'],
  },
  {
    id: 'vendor-auction',
    question: 'What happens if my property is passed in at auction?',
    answer: 'If your property does not reach the reserve price at auction, it is "passed in". The highest genuine bidder typically has the first right of negotiation — the agent will contact them immediately after the auction to try to negotiate a sale. If no agreement is reached, the property will be listed for private treaty or another sale method. Cooling-off does NOT apply to sales negotiated on auction day.',
    category: 'vendors',
    tags: ['auction', 'passed in', 'reserve', 'negotiation'],
  },
  {
    id: 'vendor-documents',
    question: 'How do I access documents related to my property sale?',
    answer: 'Your agent controls what documents are uploaded to the Document Vault. Once uploaded, you will receive an email notification. Log in to your account and visit the property page → Documents tab to access files like the Contract of Sale, vendor disclosure documents, and settlement paperwork. You can also request documents directly from your agent via the Documents section.',
    category: 'vendors',
    tags: ['documents', 'contract', 'settlement', 'vendor'],
  },
  {
    id: 'vendor-cooling-off',
    question: 'Does the cooling-off period apply to my sale?',
    answer: 'In most Australian states, a cooling-off period applies to private treaty sales (typically 3–5 business days depending on the state). However, cooling-off does NOT apply to properties sold at auction, or sold on the same day as the auction, or via an exchange after being passed in on auction day. Your agent and conveyancer will confirm the specific rules for your state.',
    category: 'vendors',
    tags: ['cooling off', 'private treaty', 'auction', 'contract'],
  },

  // AUCTIONS
  {
    id: 'auction-reserve',
    question: 'What is a reserve price?',
    answer: 'The reserve price is the minimum price the vendor will accept for their property at auction. It is set confidentially between the vendor and agent and is never disclosed to bidders or the public. When bidding reaches the reserve, the auctioneer announces the property is "on the market" — meaning it will sell to the highest bidder from that point.',
    category: 'auctions',
    tags: ['reserve', 'on the market', 'auction'],
  },
  {
    id: 'auction-vendor-bid',
    question: 'What is a vendor bid?',
    answer: 'A vendor bid is a bid made by the auctioneer on behalf of the vendor, up to the reserve price. Vendor bids are legal in all Australian states but must be clearly announced by the auctioneer. On ListHQ\'s live auction feed, vendor bids are labelled "Vendor Bid" in orange. Once the reserve is reached, no further vendor bids can be made.',
    category: 'auctions',
    tags: ['vendor bid', 'auction', 'auctioneer'],
  },
  {
    id: 'auction-cooling-off',
    question: 'Does cooling-off apply to auction purchases?',
    answer: 'No. In most Australian states, the cooling-off period does NOT apply to properties purchased at auction or on the same day as the auction. This means once the hammer falls, you are legally committed to the purchase. Make sure your finance is pre-approved and all due diligence (building inspection, legal review) is completed before bidding.',
    category: 'auctions',
    tags: ['cooling off', 'auction', 'contract', 'legal'],
  },
  {
    id: 'auction-deposit',
    question: 'How much deposit do I need at auction?',
    answer: 'The standard auction deposit in Australia is 10% of the purchase price, payable on the day. You will need to have this ready as a bank cheque or an approved direct bank transfer. Confirm the payment method with the listing agent before auction day. The balance is paid on the settlement date (typically 30–90 days after auction).',
    category: 'auctions',
    tags: ['deposit', 'auction', '10%', 'payment'],
  },
  {
    id: 'auction-live-watch',
    question: 'Can I watch an auction live online?',
    answer: 'Yes. If the agent has enabled the public live auction feed, visit the property page during the scheduled auction time and click "Watch Live". The live bid feed updates in real-time showing each bid, whether the reserve has been met, and the final outcome. You do not need to be a registered bidder to watch.',
    category: 'auctions',
    tags: ['live', 'watch', 'online', 'auction feed'],
  },
  {
    id: 'auction-multilingual',
    question: 'Can I watch the auction live in my language?',
    answer: 'Yes. On the live auction page, use the language selector in the navigation bar to switch languages. Bid notifications, reserve announcements, and auction results will appear in your selected language in real time. Your language preference is saved automatically for future visits. Supported languages include English, Mandarin (Simplified and Traditional), Vietnamese, Korean, Arabic, and Japanese.',
    category: 'auctions',
    tags: ['multilingual', 'language', 'mandarin', 'vietnamese', 'korean', 'arabic', 'japanese', 'live'],
  },
  {
    id: 'auction-watch-public',
    question: 'Do I need an account to watch a live auction?',
    answer: 'No. The public live auction feed at /auctions/[id]/live is accessible without signing in. You can watch all bids in real time, see when the reserve is met, and follow the auction result. To register as a bidder and place bids yourself, you will need to create an account and complete the registration process at least 24 hours before the auction.',
    category: 'auctions',
    tags: ['watch', 'live', 'public', 'no account', 'register'],
  },
  {
    id: 'auction-bid-realtime',
    question: 'How does the real-time bid feed work?',
    answer: 'The live bid feed uses Supabase Realtime to push each new bid to your screen the instant it is recorded by the auctioneer — no page refresh needed. You will see the bid amount, whether it is a genuine or vendor bid, and whether the reserve has been met. A notification toast appears in your chosen language with each new bid. The feed also shows if the property has SOLD or been Passed In.',
    category: 'auctions',
    tags: ['realtime', 'bid feed', 'live', 'technology', 'how it works'],
  },

  // BILLING
  {
    id: 'billing-plans',
    question: 'What are the agent subscription plans?',
    answer: 'ListHQ offers monthly subscription plans for agents. Visit our pricing page or contact hello@listhq.com.au for current pricing. All plans include unlimited property listings, access to the full CRM, open home management, auction tools, vendor reporting, and document vault.',
    category: 'billing',
    tags: ['plans', 'pricing', 'subscription'],
  },
  {
    id: 'billing-cancel',
    question: 'How do I cancel my subscription?',
    answer: 'To cancel your subscription, go to Dashboard → Billing and click "Cancel Subscription". Full subscription management including cancellation via Stripe is launching soon. In the meantime, contact hello@listhq.com.au and we will process your request within 1 business day.',
    category: 'billing',
    tags: ['cancel', 'subscription', 'billing'],
  },
  {
    id: 'billing-invoices',
    question: 'Where do I find my invoices?',
    answer: 'Invoice history will be available at Dashboard → Billing once Stripe billing launches. In the meantime, contact hello@listhq.com.au to request a record of your account. Founding agents on the free trial period will receive their first invoice when the trial converts to a paid plan.',
    category: 'billing',
    tags: ['invoices', 'receipts', 'billing', 'download'],
  },
  {
    id: 'billing-update-card',
    question: 'How do I update my payment method?',
    answer: 'Payment method management via Stripe is launching soon. Once live, you will be able to update your card at Dashboard → Billing → Payment Method. For any urgent billing changes before launch, contact hello@listhq.com.au.',
    category: 'billing',
    tags: ['payment', 'card', 'stripe', 'update'],
  },
  {
    id: 'billing-founding-agent',
    question: 'What is the Founding Agent offer?',
    answer: 'ListHQ\'s Founding Agent offer gives the first agents who join the platform 60 days of full access at no charge — no credit card required at signup.\n\nFounding Agent benefits include:\n• Full platform access for 60 days free\n• Locked-in founding pricing when you convert to a paid plan (lower than the standard public rate)\n• Founding Agent badge on your public agent profile\n• Direct input into product development — your feedback shapes what gets built next\n• Priority support during the launch period\n\nThe Founding Agent offer is available to a limited number of agents in each market. Once the founding cohort is full, new agents join at standard pricing. To claim the offer, sign up and complete your agency onboarding — no payment details are needed until the free period ends.',
    category: 'billing',
    tags: ['founding agent', 'free', 'trial', 'offer', 'pricing', 'badge', '60 days'],
  },

  // GENERAL (continued)
  {
    id: 'general-esignature',
    question: 'How does electronic document signing work?',
    answer: 'ListHQ includes an e-signature system that allows agents to send documents for electronic signing to buyers, vendors, and tenants.\n\nFor signatories (buyers, vendors, tenants):\n• You will receive an email with a "Sign Document" button\n• Click the link — no account or login is required\n• Read the document name and details\n• Type your full legal name in the signature field and click "Sign"\n• Your signature is recorded with a timestamp\n\nE-signatures on ListHQ are legally binding under the Electronic Transactions Act 1999 (Australia). Each signed document generates a tamper-evident record with the signatory\'s name, timestamp, and IP address.\n\nFor agents sending signature requests: go to Dashboard → Documents → Signature Requests → New Request. Enter the document name, optional document URL, and up to 5 signatories (name and email). Each signatory receives an email with their unique signing link.',
    category: 'general',
    tags: ['e-signature', 'electronic signature', 'signing', 'documents', 'legal', 'Electronic Transactions Act'],
  },
  {
    id: 'general-offmarket-vs-premarket',
    question: 'What is the difference between an off-market listing and a pre-market listing?',
    answer: 'ListHQ supports two types of private listings:\n\n• Off-Market — the property is not advertised publicly at all. It is visible only to buyers who have subscribed to off-market alerts for that suburb and price range. Off-market listings do not appear in search results. Agents use them for discreet sales where the vendor does not want public exposure.\n\n• Pre-Market (Coming Soon) — the property is publicly visible in search results but marked as "Coming Soon". Buyers can register their interest, but the property is not yet open for inspections or offers. Agents use this to build a buyer list before going live, often 1–2 weeks before the official launch date.\n\nIn both cases, the agent controls who sees the full listing details. Pre-market listings transition to a full active listing on the launch date.',
    category: 'general',
    tags: ['off-market', 'pre-market', 'coming soon', 'private', 'listing type', 'difference'],
  },

  // TECHNICAL
  {
    id: 'tech-browser',
    question: 'Which browsers does ListHQ support?',
    answer: 'ListHQ supports the latest two versions of Chrome, Safari, Firefox, and Edge. For the best experience, we recommend using Chrome or Safari. Mobile browsers on iOS (Safari) and Android (Chrome) are fully supported.',
    category: 'technical',
    tags: ['browser', 'compatibility', 'safari', 'chrome'],
  },
  {
    id: 'tech-mobile',
    question: 'Is there a mobile app?',
    answer: 'ListHQ is a Progressive Web App (PWA) — it works on all mobile browsers and can be installed on your home screen like a native app.\n\nOn iOS (Safari): tap the Share button → "Add to Home Screen"\nOn Android (Chrome): tap the three-dot menu → "Add to Home Screen" or "Install App"\n\nOnce installed, ListHQ opens full-screen without browser chrome, loads faster on repeat visits, and works as a standalone app icon on your device. No app store download required.',
    category: 'technical',
    tags: ['mobile', 'app', 'ios', 'android', 'pwa'],
  },
  {
    id: 'tech-notifications',
    question: 'How do I manage my email notification preferences?',
    answer: 'Go to your account settings → Notifications. You can toggle on or off: new listing alerts, price drop alerts, open home reminders, auction results, message notifications, and weekly suburb reports. To unsubscribe from all marketing emails, click the unsubscribe link at the bottom of any email.',
    category: 'technical',
    tags: ['notifications', 'email', 'preferences', 'unsubscribe'],
  },
  {
    id: 'tech-data-privacy',
    question: 'How does ListHQ handle my personal data?',
    answer: 'ListHQ collects and stores personal information in accordance with the Australian Privacy Act 1988 and the Australian Privacy Principles. Your data is stored securely on Australian servers. We never sell your personal information to third parties. See our full Privacy Policy at listhq.com.au/privacy for details.',
    category: 'technical',
    tags: ['privacy', 'data', 'gdpr', 'privacy act'],
  },
  {
    id: 'tech-delete-account',
    question: 'How do I delete my account?',
    answer: 'Go to Account Settings → Privacy → Delete Account. For agent accounts with active listings, you must first deactivate all listings before deletion. Account deletion is permanent and cannot be undone. Alternatively, contact support@listhq.com.au and we\'ll process the deletion within 30 days as required by the Privacy Act.',
    category: 'technical',
    tags: ['delete account', 'privacy', 'data removal'],
  },
  {
    id: 'tech-listing-wrong-language',
    question: 'Why is my listing showing in the wrong language or not translating?',
    answer: 'There are a few reasons a listing may not display correctly in a non-English language:\n\n1. Translation still processing — after publishing, translations typically complete within 1–2 minutes. If the language toggle is not yet showing, wait a moment and refresh the page.\n\n2. Description too short — the AI translation requires a minimum description length to produce a quality translation. If your listing description is under 50 words, the translation may be incomplete. Add more detail and trigger a re-translation from the listing detail page.\n\n3. Re-translation needed — if you have significantly updated your listing description after the initial translation, the translated versions may be outdated. Trigger a fresh translation from the listing detail page → Translations tab → "Regenerate Translations".\n\n4. Language selector not responding — if the language selector in the navigation bar is not opening, this may be a z-index display issue on certain pages. Try refreshing the page, or contact support@listhq.com.au.',
    category: 'technical',
    tags: ['translation', 'language', 'wrong language', 'multilingual', 'bug', 'not working'],
  },

  // ── Buying Costs / FIRB ──
  {
    id: 'buyer-firb-calculator',
    question: 'How does the Buying Costs Calculator (FIRB) work?',
    answer: 'Every property listing page includes a "Buying Costs Calculator" section that estimates your total upfront purchase costs.\n\nWhat it estimates:\n• Standard stamp duty (state-by-state)\n• FIRB application fees (foreign buyers)\n• Foreign investor duty surcharges\n• Legal and conveyancing costs (indicative)\n\nHow to use it: scroll to the "Buying Costs Calculator" on any listing. By default it shows costs for an Australian resident. Toggle "I am a foreign buyer" on and the FIRB application fee and foreign investor duty surcharge are automatically added to the total.\n\nFIRB application fees by purchase price:\n• Under $75,000 — $4,200\n• $75,000–$1M — $13,200\n• $1M–$2M — $26,400\n• $2M–$3M — $52,800\n• Over $3M — $79,200\n\nForeign investor duty surcharge by state:\n• NSW 8% • VIC 8% • QLD 7% • SA 7% • WA 7% • TAS 8% • ACT 7% • NT 0%\n\nFigures are indicative only; verify final amounts at firb.gov.au and with a solicitor.',
    category: 'buyers',
    tags: ['firb', 'stamp duty', 'foreign buyer', 'calculator', 'costs', 'overseas'],
  },

  // ── Sharing properties ──
  {
    id: 'buyer-share-property',
    question: 'How do I share a property with friends or family?',
    answer: 'Tap the share button (arrow icon) on any property listing. You\'ll see four options:\n\n• Copy link — copies the property URL to your clipboard so you can paste it anywhere\n• WhatsApp — opens WhatsApp with the property details pre-filled\n• Line — opens Line messenger to share the property\n• WeChat — displays a QR code. Open WeChat on your phone → tap Scan → point your camera at the QR code to share with your contacts',
    category: 'buyers',
    tags: ['share', 'wechat', 'whatsapp', 'line', 'qr code', 'copy link'],
  },
  {
    id: 'buyer-share-wechat',
    question: 'How do I share a property on WeChat?',
    answer: 'Open any property listing and tap the share button. Select WeChat and a QR code will appear — open WeChat on your phone, tap Scan, and point your camera at the code to share it with your contacts.',
    category: 'buyers',
    tags: ['wechat', 'share', 'qr code', 'china'],
  },

  // ── Language & Currency ──
  {
    id: 'buyer-language-currency',
    question: 'How do I change the language or currency?',
    answer: 'ListHQ supports 25 languages and 13+ currencies.\n\nLanguage:\n• On your first visit, the site automatically detects your browser language\n• To change it later, tap the language selector (globe icon) in the top navigation\n• Available in English, Chinese (Simplified & Traditional), Japanese, Korean, Vietnamese, Thai, Indonesian, Hindi, Arabic, Spanish, French, German, and many more\n\nCurrency:\n• Tap the currency selector next to the language button\n• Prices display in AUD, USD, CNY, HKD, SGD, JPY and more — converted instantly using current exchange rates\n\nYour language and currency preferences are saved automatically and remembered on every visit. The language selector shows all available languages — newly supported languages including Hindi and Bengali are now available. The language selector shows all available languages — newly supported languages including Hindi and Bengali are now available.',
    category: 'buyers',
    tags: ['language', 'currency', 'translation', 'aud', 'usd', 'cny', 'international'],
  },
  {
    id: 'buyer-international',
    question: 'Can international buyers use ListHQ?',
    answer: 'Yes. ListHQ is built for global buyers. The site supports 25 languages, 13 currencies, and includes a FIRB (Foreign Investment Review Board) calculator to estimate all buying costs including foreign investor surcharges and application fees. Many listings also include translated titles and descriptions in Chinese, Japanese, and Korean added by the listing agent.',
    category: 'buyers',
    tags: ['international', 'overseas', 'foreign buyer', 'global', 'firb'],
  },
  {
    id: 'buyer-currency-display',
    question: 'Can I view listings in my own currency?',
    answer: 'Yes — use the currency selector in the top navigation to switch between AUD, USD, CNY, HKD, SGD, JPY and more. Prices update instantly across the entire site using live exchange rates.',
    category: 'buyers',
    tags: ['currency', 'aud', 'usd', 'cny', 'exchange rate'],
  },
  {
    id: 'buyer-listings-chinese',
    question: 'Are listings available in Chinese?',
    answer: 'Many listings include Chinese (Simplified and Traditional) titles and descriptions added directly by the agent during listing creation. The site also auto-translates listing content when you browse in Chinese — switch your language using the globe icon in the top navigation.',
    category: 'buyers',
    tags: ['chinese', 'translation', 'mandarin', 'multilingual'],
  },
  {
    id: 'buyer-ai-search',
    question: 'How does AI property search work?',
    answer: 'AI search lets you describe what you are looking for in plain language instead of filling in filters. Toggle "AI search" on the Buy page, type a natural description into the search bar, and click "Find My Home".\n\nExamples of queries that work:\n• "3 bedroom house with a backyard near good schools under $900k"\n• "Apartment close to the city with parking, pet-friendly, under $600k"\n• "Family home in Templestowe or Doncaster with a pool"\n\nThe AI reads your description, extracts suburb, bedroom count, property type, budget, and lifestyle requirements, then searches the database to match listings. Results are returned ranked by relevance.\n\nYou can search in English or your own language — the AI will translate your query automatically before searching.',
    category: 'buyers',
    tags: ['AI search', 'natural language', 'search', 'find my home', 'smart search'],
  },
  {
    id: 'buyer-ai-search-language',
    question: 'Can I search for properties in Korean, Arabic, Chinese, or Hindi?',
    answer: 'Yes. The AI search bar accepts queries in any language. Type your property requirements in Korean, Arabic, Chinese, Vietnamese, Japanese, Hindi, Bengali, or any other language — the system will automatically translate your query into English before searching, so you get the same quality results as an English search.\n\nFor example, you can type "3개의 침실이 있는 집" (Korean for "3 bedroom house") and the AI will understand your intent and return matching listings.\n\nOr type "হিন্দিতে 3 বেডরুমের বাড়ি" (Bengali for "3 bedroom house") — the AI understands both.\n\nYou can also switch the entire site to your preferred language using the language selector in the top navigation bar. Listing titles and descriptions will display in your chosen language where translations are available.',
    category: 'buyers',
    tags: ['korean', 'arabic', 'chinese', 'hindi', 'bengali', 'language', 'search', 'multilingual', 'AI search'],
  },
  {
    id: 'buyer-translated-listings',
    question: 'Are property listings translated into my language?',
    answer: 'Yes. When you switch the site language using the selector in the top navigation bar, listing titles and descriptions are automatically displayed in your chosen language.\n\nCurrently supported languages for listing translations:\n• 简体中文 — Mandarin Simplified\n• 繁體中文 — Mandarin Traditional\n• 한국어 — Korean\n• العربية — Arabic\n• 日本語 — Japanese\n• Tiếng Việt — Vietnamese\n• हिन्दी — Hindi\n• বাংলা — Bengali\n\nTranslations are generated by AI and cached — the first time a listing is viewed in your language, it may take a few seconds to translate. Every subsequent view is instant.\n\nFor English listings, you may also see Cultural Highlights — a short section written specifically for buyers from your community, noting features like school proximity, orientation, garden space, or suburb demographics that are most relevant to your background.',
    category: 'buyers',
    tags: ['translation', 'language', 'korean', 'arabic', 'chinese', 'japanese', 'multilingual', 'listings', 'hindi', 'bengali'],
  },
  {
    id: 'buyer-anonymous-contact',
    question: 'How do I get contacted by an agent about a property I searched for?',
    answer: 'When you search for a property using AI search, ListHQ anonymously records your search intent (the type of property you are looking for, not your personal details). Agents whose listings match your search may see your intent — things like "buyer looking for 3 bedrooms in Templestowe under $900k" — and may reach out.\n\nIf an agent contacts you, you will receive a message through the platform. Your name, email address, and phone number are never shared with the agent unless you choose to reply and provide them directly.\n\nYou can opt out of buyer matching at any time from your account settings.',
    category: 'buyers',
    tags: ['contact', 'agent', 'privacy', 'anonymous', 'buyer matching', 'AI'],
  },
  {
    id: 'agent-translate-step',
    question: 'How do I add Chinese, Japanese, or Korean translations to my listing?',
    answer: 'When creating a listing, the new "Translate" step (step 4 in the pocket listing wizard) lets you add translated versions of your title and description in:\n\n• Chinese (Simplified)\n• Chinese (Traditional)\n• Japanese\n• Korean\n\nClick "Auto-translate from English" to generate translations instantly using AI, or type them manually for full control.\n\nBuyers browsing in Chinese, Japanese, or Korean will automatically see your translated version of the listing instead of the auto-translation.\n\nThis step is optional — you can skip it and still publish. You can return to any draft and add translations later.',
    category: 'agents',
    tags: ['translate', 'chinese', 'japanese', 'korean', 'multilingual', 'pocket listing', 'wizard'],
  },

  // ── International Referral Program ──
  {
    id: 'referral-program-overview',
    question: 'What is the International Referral Program?',
    answer: 'The ListHQ International Referral Program lets agents and advisors anywhere in the world earn commissions by connecting international buyers with Australian property.\n\nEarnings: AUD $500–$2,000 per settled referral, depending on your tier.\n\nHow to join: visit listhq.com.au/refer, fill in your details, and receive your unique referral code instantly. There is no fee to join.\n\nWho it\'s for: international real estate agents, migration consultants, financial advisors, and anyone with networks of buyers interested in Australian property.',
    category: 'agents',
    tags: ['referral', 'international', 'commission', 'agents', 'overseas'],
  },
  {
    id: 'referral-dashboard',
    question: 'How do I track my referrals and commissions?',
    answer: 'After signing up at listhq.com.au/refer, log into your Referral Dashboard to:\n\n• Track every referred buyer and their progress (new → contacted → viewing → offer → settled)\n• Monitor commission earnings in real time\n• See your current tier and progress toward the next one\n• Submit new referrals at any time using the "Submit a referral" button',
    category: 'agents',
    tags: ['referral', 'dashboard', 'tracking', 'commission'],
  },
  {
    id: 'referral-sharing',
    question: 'How do I share my referral code?',
    answer: 'From your Referral Dashboard you have several sharing options:\n\n• Copy your unique referral link\n• Share via WhatsApp with a pre-filled message\n• Share via Line messenger\n• Show a WeChat QR code — buyers scan to open your link\n\nAny buyer who registers or enquires through your link is automatically attributed to you.',
    category: 'agents',
    tags: ['referral', 'share', 'wechat', 'whatsapp', 'line', 'qr code'],
  },
  {
    id: 'referral-tiers',
    question: 'How do referral commission tiers work?',
    answer: 'Your tier rises automatically as your referrals settle:\n\n• Standard — AUD $500 per settled referral (default starting tier)\n• Silver — AUD $750 per settled referral\n• Gold — AUD $1,000 per settled referral\n• Platinum — AUD $2,000 per settled referral\n\nTier is based on your total count of settled referrals. Once promoted, all future settled referrals earn at the new rate.',
    category: 'agents',
    tags: ['referral', 'tier', 'commission', 'silver', 'gold', 'platinum'],
  },

  // PROPERTY MANAGEMENT
  {
    id: 'pm-tenant-portal-access',
    question: 'How do I give my tenant access to their portal?',
    answer: 'Go to your agent dashboard → open the tenancy → find the Tenant Portal card. Click "Copy link" or "Email to tenant" to send them their personal portal link. They can view their lease, rent payments, submit maintenance requests and access documents — no login required.',
    category: 'property-managers',
    tags: ['tenant portal', 'tenancy', 'access', 'link'],
  },
  {
    id: 'pm-owner-portal-access',
    question: 'How do I give my landlord/owner access to their portal?',
    answer: 'Open the property or tenancy in your dashboard → find the Owner Portal card → click "Copy link" or "Email to owner." Owners can view financials, approve maintenance quotes, download statements and view inspection reports — no login required.',
    category: 'property-managers',
    tags: ['owner portal', 'landlord', 'access', 'link'],
  },
  {
    id: 'pm-arrears-automation',
    question: 'How do automated arrears reminders work?',
    answer: 'When a tenant falls overdue, the platform automatically sends reminders at day 1, 3, 7 and 14. You can turn this on or off in Dashboard → Automation Settings, and edit the email templates and merge fields such as {tenant_name}, {property_address} and {amount_overdue}.',
    category: 'property-managers',
    tags: ['arrears', 'automation', 'reminders', 'overdue'],
  },
  {
    id: 'pm-maintenance-management',
    question: 'How does maintenance request management work?',
    answer: 'Tenants submit requests via their portal link. You receive them in Dashboard → Maintenance, where you can assign a tradesperson, request owner approval for large jobs, track status, and upload invoices. Tenants are automatically notified when their job status changes.',
    category: 'property-managers',
    tags: ['maintenance', 'jobs', 'tradesperson', 'workflow'],
  },
  {
    id: 'pm-supplier-portal',
    question: 'Can suppliers access job details without logging in?',
    answer: 'Yes. From the Suppliers page, copy or email the supplier\'s portal link. They can view assigned jobs, accept them, propose a schedule, mark complete, and upload their invoice — all without an account.',
    category: 'property-managers',
    tags: ['supplier', 'portal', 'tradesperson', 'jobs'],
  },
  {
    id: 'pm-share-whatsapp-line',
    question: 'How do I share a listing on WhatsApp or Line?',
    answer: 'Tap the share button on any property listing. Select WhatsApp to send directly with property details pre-filled, or Line to share to your Line contacts. WeChat sharing uses a QR code — open WeChat → Scan → point at the QR code.',
    category: 'general',
    tags: ['share', 'whatsapp', 'line', 'wechat'],
  },
  {
    id: 'pm-vacancy-kpi',
    question: 'What is the vacancy KPI dashboard?',
    answer: 'Dashboard → Vacancy KPIs shows your portfolio occupancy rate, properties currently vacant, vacating this month, average days to re-let, and total vacancy loss in AUD. A 90-day timeline view shows all properties and their lease status at a glance.',
    category: 'property-managers',
    tags: ['vacancy', 'kpi', 'occupancy', 'dashboard'],
  },

  // PROPERTY MANAGEMENT — DETAILED
  {
    id: 'pm-getting-started',
    question: 'How do I access property management features?',
    answer: 'From your agent dashboard, look for the "Property Management" section in the left sidebar. It includes Rent Roll, Rental Applications, Maintenance, Vacancies, Vacancy KPIs, Suppliers, Statements and Automation. Trust accounting and inspections live alongside these tools.',
    category: 'property-managers',
    tags: ['getting started', 'sidebar', 'navigation'],
  },
  {
    id: 'pm-rent-roll',
    question: 'How does the rent roll work?',
    answer: 'Dashboard → Rent Roll shows every active tenancy with lease dates, weekly rent, paid-to date and arrears status. Click any tenancy to record a payment (amount, date, method), view its full history, or send the tenant their portal link. Lease expiries within 60 days are colour-coded with amber and red warnings, and you can filter or search by suburb, tenant name or status.',
    category: 'property-managers',
    tags: ['rent roll', 'tenancy', 'rent payment', 'lease expiry'],
  },
  {
    id: 'pm-trust-accounting',
    question: 'How does trust accounting work?',
    answer: 'The Trust Accounting page shows your trust account balance, BSB and account number, and every transaction. Record receipts (rent, deposits, owner contributions) and payments (owner disbursements, supplier invoices, fees). Each transaction has a status — pending, cleared, reconciled or voided — and once reconciled it cannot be edited, only voided with an audit trail.',
    category: 'property-managers',
    tags: ['trust accounting', 'transactions', 'reconciliation', 'compliance'],
  },
  {
    id: 'pm-arrears-detail',
    question: 'How are arrears calculated and managed?',
    answer: 'Arrears are calculated as days overdue × daily rent based on the tenant\'s paid-to date. The Arrears dashboard groups tenants by severity (1–7, 8–14, 15+ days). Tap "Remind" to send an immediate manual email, or enable automated sequences that send at day 1, 3, 7 and 14. Toggle automation on/off in Dashboard → Automation Settings, and view every automated send in the recent sends log.',
    category: 'property-managers',
    tags: ['arrears', 'overdue', 'reminders', 'automation'],
  },
  {
    id: 'pm-lease-renewals',
    question: 'How do lease renewals work?',
    answer: 'Tenancies show renewal status badges: amber "Due Soon" at 60–90 days from expiry and red "Urgent" under 60 days. Open a tenancy and click "Offer Renewal" to set the new rent, new end date and lease type (fixed or periodic). Mark the offer as accepted to update the tenancy, or declined to automatically trigger the vacancy workflow.',
    category: 'property-managers',
    tags: ['lease renewal', 'expiry', 'tenancy', 'vacancy'],
  },
  {
    id: 'pm-inspections',
    question: 'How do I schedule and conduct inspections?',
    answer: 'Dashboard → Inspections lets you schedule Entry (ingoing), Routine, and Exit (outgoing) inspections. The system checks your state\'s minimum notice period when you enter a date and warns you if it\'s too soon.\n\n**Conducting an inspection**\nOpen the inspection on the day. Walk room-by-room and record condition (Excellent / Good / Fair / Poor / Damaged), add notes, and upload photos from your camera — photos are stored securely against the tenancy. Add any rooms not on the default list using the quick-add chips (Pool, Balcony, Granny Flat, etc.) or type a custom name.\n\n**Finalising and sending**\nTap "Finalise Report" when done. The dialog lets you:\n • Send to Owner + Tenant simultaneously\n • Send to Owner only\n • Send to Tenant only\n • Finalise without sending\n\nOnce finalised, a Download PDF button appears to export the full report with all rooms, photos, maintenance items, and key/meter/smoke alarm data.\n\n**Exit inspections — Routine History panel**\nOn Exit inspections, a Routine History panel automatically appears showing every completed routine inspection for that tenancy, with room-by-room condition and notes. This gives a clear comparison of how the property\'s condition has changed over the tenancy — making fair bond claim assessment much easier.\n\n**Calendar view**\nThe Calendar tab (between Upcoming and Overdue on the Inspections page) shows all scheduled inspections on a colour-coded monthly calendar: blue = Entry, teal = Routine, orange = Exit. Click any pill to open the report.\n\n**Auto-schedule annual routines**\nThe Auto-Schedule Routines button bulk-plans routine inspections across your entire portfolio. It calculates how many routines each tenancy still needs to reach 4 per year, plans dates ~91 days apart from the last inspection, and respects each state\'s minimum notice period. A preview shows proposed dates before anything is confirmed.',
    category: 'property-managers',
    tags: ['inspections', 'condition report', 'entry notice', 'photos', 'pdf', 'exit inspection', 'routine history', 'calendar', 'auto-schedule', 'custom rooms'],
  },
  {
    id: 'pm-maintenance-detail',
    question: 'How does the maintenance workflow work end-to-end?',
    answer: 'Tenants submit requests via their portal link. You see them in Dashboard → Maintenance, filtered by status (new, quoted, scheduled, in progress, complete). Assign a tradesperson from your supplier list (filtered by trade category) and the supplier is emailed with job details and the property address. Status changes auto-notify the tenant. Upload invoices, record final costs, and request owner approval for any job above the threshold.',
    category: 'property-managers',
    tags: ['maintenance', 'workflow', 'suppliers', 'invoices'],
  },
  {
    id: 'pm-suppliers',
    question: 'How do I manage suppliers?',
    answer: 'Add suppliers with trade category, ABN, license number and insurance expiry date. Insurance alerts go amber 30–60 days from expiry and red under 30 days. Assign jobs from the Suppliers page or Maintenance dashboard. Performance is tracked automatically: rate suppliers after each completed job and view their average response time. Suppliers can update job status via their secure portal link without an account.',
    category: 'property-managers',
    tags: ['suppliers', 'tradesperson', 'insurance', 'rating'],
  },
  {
    id: 'pm-vacancies',
    question: 'How do I manage vacancies?',
    answer: 'Vacancies progress through five statuses: Notice Given → Vacating → Vacant → Re-listed → Re-let. Update each from the Vacancies dashboard. The Vacancy KPI dashboard shows occupancy rate, average days to re-let and total vacancy loss in AUD, with a 90-day timeline view across your whole portfolio. Automated alerts fire on day 7, 14 and 21 if a property is vacant with no active listing.',
    category: 'property-managers',
    tags: ['vacancy', 'kpi', 'occupancy', 'alerts'],
  },
  {
    id: 'pm-owner-statements',
    question: 'How do I create owner statements?',
    answer: 'Go to Dashboard → Statements → New Statement. Select the property, set the period, and enter gross rent collected, your management fee percentage and any maintenance costs. Net to owner is calculated automatically. Email the statement directly to the owner from the platform. All historical statements are stored, and a monthly automated reminder fires if no statement was created for the previous month.',
    category: 'property-managers',
    tags: ['statements', 'owner', 'financials', 'management fee'],
  },
  {
    id: 'pm-automation-settings',
    question: 'What automations can I configure?',
    answer: 'Dashboard → Automation Settings exposes four automation types: arrears sequences, lease renewal notices, inspection entry notices and maintenance status updates. Edit each template using merge fields such as {tenant_name}, {property_address} and {amount_overdue}. Use the "Run now" button to trigger any automation immediately for testing, and review the last 20 automated sends with delivery status in the recent sends log.',
    category: 'property-managers',
    tags: ['automation', 'templates', 'merge fields', 'emails'],
  },
  {
    id: 'pm-tenant-portal-detail',
    question: 'What can tenants see in their portal?',
    answer: 'The tenant portal shows their lease summary, rent payment history, open and past maintenance requests, shared documents, and inspection reports. They can submit new maintenance requests directly. Send the link from Tenancy Detail → Tenant Portal card → Copy link or Email to tenant. The link is unique and secure — no account or login required.',
    category: 'property-managers',
    tags: ['tenant portal', 'lease', 'maintenance', 'documents'],
  },
  {
    id: 'pm-owner-portal-detail',
    question: 'What can owners see in their portal and how do approvals work?',
    answer: 'The owner portal shows a property overview, financials, pending maintenance approvals, statements and inspection reports. Send the link from the property or tenancy detail → Owner Portal card → Copy link or Email to owner. Owners approve or decline maintenance jobs above the approval threshold. Set the threshold per property in property settings → maintenance approval threshold (default $500). All access is via a secure token link with no login required.',
    category: 'property-managers',
    tags: ['owner portal', 'approval', 'threshold', 'maintenance'],
  },
  {
    id: 'pm-supplier-portal-detail',
    question: 'What can suppliers do in their portal?',
    answer: 'Suppliers see their assigned jobs, full job details, tenant contact for access arrangements, and scheduled dates. They can accept jobs, propose a schedule, mark them complete and upload invoices — all without an account. Send the portal link from the Suppliers page → supplier card → Copy portal link.',
    category: 'property-managers',
    tags: ['supplier portal', 'jobs', 'invoices', 'tradesperson'],
  },

  // HALO — SEEKER SIDE
  {
    id: 'halo-what-is',
    question: 'What is Halo?',
    answer: 'Halo is a reverse property marketplace — instead of you searching for properties, agents search for you. Create a Halo brief describing exactly what you want: suburb, price range, property type, bedrooms, finance status, and any must-haves. Matching agents can see your requirements without your personal details and reach out to you directly through the platform. You stay anonymous until you choose to respond.',
    category: 'halo',
    tags: ['halo', 'what is', 'reverse marketplace', 'overview'],
  },
  {
    id: 'halo-how-create',
    question: 'How do I create a Halo?',
    answer: 'Visit listhq.com.au/halo and click "Create Your Halo". The 3-step wizard takes under 3 minutes: Step 1 covers your buying or renting intent, property type, and bedroom requirements. Step 2 covers suburbs, budget, and location flexibility. Step 3 covers your timeframe, finance status, must-haves, deal breakers, and preferred contact language. Your draft is saved automatically if you stop partway through — you can return and complete it at any time.',
    category: 'halo',
    tags: ['halo', 'create', 'wizard', 'brief'],
  },
  {
    id: 'halo-wizard-steps',
    question: 'What information do I need to create a Halo?',
    answer: 'You need to know: whether you\'re buying or renting, which property types you\'re open to (house, apartment, townhouse, etc.), your preferred suburbs and whether you\'re flexible on location, your budget range, your timeframe (ready now, 1–3 months, 3–6 months, or flexible), and your finance status (not started, pre-approval in progress, or pre-approved). You can also add must-haves like a pool or garage, and deal breakers like a busy road. Everything except the basics is optional — a rough suburb and budget is enough to get started.',
    category: 'halo',
    tags: ['halo', 'information', 'wizard', 'requirements', 'finance'],
  },
  {
    id: 'halo-manage',
    question: 'How do I manage, pause, or delete my Halos?',
    answer: 'Log in and go to My Dashboard (listhq.com.au/seeker/dashboard). Each Halo appears as a card showing its status (active, paused, or expired), how many agents have responded, and how many unread responses you have. Click Edit to update your requirements — useful if your budget changes or you want to add suburbs. Use Pause to temporarily stop new agent responses without deleting your Halo. Use Delete to remove it permanently. Paused Halos can be reactivated at any time.',
    category: 'halo',
    tags: ['halo', 'manage', 'pause', 'delete', 'edit', 'dashboard'],
  },
  {
    id: 'halo-responses',
    question: 'How do I see which agents have responded to my Halo?',
    answer: 'Go to My Dashboard (listhq.com.au/seeker/dashboard) and look at the response count on each Halo card. Click View Responses to see which agents have reached out, what properties or areas they\'re suggesting, and their message. Your personal contact details — name, email, and phone — are never shared with agents without your consent. You control when and whether to respond to any agent.',
    category: 'halo',
    tags: ['halo', 'responses', 'agents', 'privacy', 'contact'],
  },
  {
    id: 'halo-seeker-inbox',
    question: 'How does the Halo inbox work?',
    answer: 'Your Halo inbox (listhq.com.au/seeker/inbox) shows all agent messages in one place. Agents who see your Halo brief can send you a message without seeing your name, email, or phone number first. When you reply, you choose how much contact information to share. Messages are organised by Halo so you can easily compare responses from different agents for different property searches.',
    category: 'halo',
    tags: ['halo', 'inbox', 'messages', 'agents', 'privacy'],
  },
  {
    id: 'halo-vs-search',
    question: 'What is the difference between Halo and regular property search?',
    answer: 'Regular search: you browse listings and contact agents. Halo: agents contact you. With a Halo brief active, agents whose listings match your requirements can reach out proactively — including with off-market properties that never appear in public search results. This is particularly valuable in competitive suburbs where good properties sell before they\'re widely advertised. You can use both at the same time — there\'s no reason to choose one or the other.',
    category: 'halo',
    tags: ['halo', 'vs search', 'difference', 'off-market', 'reverse marketplace'],
  },
  {
    id: 'halo-free',
    question: 'Is creating a Halo free?',
    answer: 'Yes. Creating a Halo is completely free for buyers and renters. You can create, edit, pause, and delete Halos at no cost. Agents pay Halo Credits to unlock your contact details — you never pay anything to use the Halo system.',
    category: 'halo',
    tags: ['halo', 'free', 'cost', 'credits', 'pricing'],
  },

  // HALO — AGENT SIDE
  {
    id: 'halo-board-what',
    question: 'What is the Halo Board?',
    answer: 'The Halo Board (Dashboard → Halo Board) is your live feed of buyer and renter briefs posted by people actively looking for property in your market. Each brief shows the buyer\'s requirements — suburb preferences, property type, bedroom count, budget range, timeframe, and finance status — without exposing their personal details. Browse and filter the board to find buyers whose needs match your listings. When you find a match, spend Halo Credits to unlock their contact details and reach out directly.',
    category: 'agents',
    tags: ['halo board', 'buyer briefs', 'halo', 'credits', 'leads'],
  },
  {
    id: 'halo-credits-what',
    question: 'What are Halo Credits?',
    answer: 'Halo Credits are the currency agents use to unlock buyer contact details on the Halo Board. Each buyer brief costs one credit to unlock — revealing the buyer\'s name, email, and phone number so you can reach out directly. Credits are purchased in packages and never expire. Browsing the Halo Board and reading briefs is always free — you only spend a credit when you actively choose to unlock a specific buyer.',
    category: 'agents',
    tags: ['halo credits', 'credits', 'halo board', 'unlock', 'pricing'],
  },
  {
    id: 'halo-credits-buy',
    question: 'How do I buy Halo Credits?',
    answer: 'Go to Dashboard → Halo Board → Buy Credits, or Dashboard → Billing → Halo Credits. Select the credit package that suits you and complete the Stripe checkout. Credits are added to your account instantly on payment confirmation. Your current credit balance is shown at the top of the Halo Board at all times.',
    category: 'agents',
    tags: ['halo credits', 'buy', 'purchase', 'stripe', 'billing'],
  },
  {
    id: 'halo-credits-cost',
    question: 'How many credits does it cost to unlock a buyer?',
    answer: 'Each buyer brief costs 1 Halo Credit to unlock. Once unlocked, you have permanent access to that buyer\'s contact details — there\'s no recurring cost per buyer. Briefs you have already unlocked are marked on the Halo Board so you never spend a credit twice on the same buyer.',
    category: 'agents',
    tags: ['halo credits', 'cost', 'unlock', 'one credit', 'pricing'],
  },
  {
    id: 'halo-board-browse',
    question: 'How do I browse and filter buyer briefs on the Halo Board?',
    answer: 'From Dashboard → Halo Board, use the filters at the top to narrow by: intent (buying or renting), suburb, budget range, property type, number of bedrooms, and buyer timeframe. Briefs are sorted by recency by default. Each card shows the buyer\'s high-level requirements without personal details. Briefs you\'ve already unlocked are clearly marked so you can see at a glance which buyers you\'re already in contact with.',
    category: 'agents',
    tags: ['halo board', 'browse', 'filter', 'buyer briefs', 'search'],
  },
  {
    id: 'halo-board-contact',
    question: 'How do I contact a buyer from the Halo Board?',
    answer: 'Find a brief that matches your listings or area, then click Unlock Contact to spend 1 Halo Credit and reveal the buyer\'s name, email, and phone number. The buyer is notified that an agent has viewed their brief — this often prompts them to check their inbox. Reach out by email or phone directly, or send an initial message through the platform before committing to a call.',
    category: 'agents',
    tags: ['halo board', 'contact', 'unlock', 'buyer', 'message'],
  },

  // RENTERS — EXPANDED
  {
    id: 'renter-decline',
    question: 'What happens if my rental application is declined?',
    answer: 'If your application is declined, you will receive an email notification from the property manager. Agents are not required to give a reason for declining, but you can ask — many will provide feedback to help your next application. Common reasons include strong competing applications, income-to-rent ratio, rental history, or reference checks. Being declined for one property does not affect your profile on the platform — you can immediately apply for other listings.',
    category: 'renters',
    tags: ['declined', 'application', 'rejection', 'feedback'],
  },
  {
    id: 'renter-bond-lodgement',
    question: 'How is my bond lodged and who holds it?',
    answer: 'Under Australian tenancy law, your bond must be lodged with the relevant state authority — not held by the agent or landlord. NSW: NSW Fair Trading (within 10 business days). VIC: Residential Tenancies Bond Authority — RTBA (within 10 business days). QLD: Residential Tenancies Authority — RTA (within 10 business days). WA: Department of Mines, Industry Regulation and Safety (within 3 business days). Once lodged, the bond can only be released with both parties\' written agreement or a tribunal order at the end of the tenancy.',
    category: 'renters',
    tags: ['bond', 'lodgement', 'rtba', 'rta', 'fair trading', 'state authority'],
  },
  {
    id: 'renter-condition-report',
    question: 'What is a condition report and why does it matter?',
    answer: 'A condition report documents the state of the property when you move in — room by room, item by item. It is your legal protection at the end of the tenancy. If the property has a scratch on the wall or a stain on the carpet when you move in, it must be on the entry condition report — otherwise you may be charged for it when you leave. Review the report carefully, note any discrepancies, and return it signed within the timeframe required in your state (typically 3–5 business days). Keep a copy for the full duration of your tenancy.',
    category: 'renters',
    tags: ['condition report', 'entry report', 'bond', 'legal', 'tenant'],
  },
  {
    id: 'renter-maintenance-request',
    question: 'How do I request repairs or maintenance as a tenant?',
    answer: 'Your property manager will send you a tenant portal link when your lease is set up. Use the portal to submit maintenance requests — describe the issue, add photos, and flag urgency (emergency or routine). Emergency repairs (gas leak, burst pipe, no hot water in winter, electrical danger) must be actioned within 24 hours under Australian law. If you cannot reach your agent in an emergency, you have the right to arrange urgent repairs up to the statutory limit for your state and recover the cost from the landlord.',
    category: 'renters',
    tags: ['maintenance', 'repairs', 'emergency', 'tenant portal', 'urgent'],
  },
  {
    id: 'renter-save-search',
    question: 'Can I save rental searches and get alerts?',
    answer: 'Yes. Run a search with your rental filters — suburb, rent range, bedrooms, pet-friendly, available date — and click Save Search. You\'ll receive an email alert every time a new rental listing matches your criteria. Set the frequency to instant, daily, or weekly. Instant alerts are particularly useful in competitive rental markets where quality properties are taken within hours of listing.',
    category: 'renters',
    tags: ['saved search', 'alerts', 'notifications', 'rental', 'email'],
  },
  {
    id: 'renter-references',
    question: 'What references do I need for a rental application?',
    answer: 'Most agents require two references: a current or previous landlord or property manager to confirm your rental history, and a personal or professional reference — an employer, colleague, or someone who can speak to your character (not a family member). Prepare your references before applying and let them know to expect a call — many applications are decided within 24–48 hours of submission, and an uncontactable reference can cost you the property.',
    category: 'renters',
    tags: ['references', 'landlord reference', 'application', 'rental history'],
  },
  {
    id: 'renter-income-proof',
    question: 'What income documents are required for a rental application?',
    answer: 'Standard requirements: the last 2–3 payslips from your employer, a current employment contract or letter confirming your position and salary, or — for self-employed applicants — the last 2 years of tax returns and an accountant\'s letter. If you receive Centrelink payments, a recent payment summary is accepted. As a general guide, your gross income should be at least 3 times the weekly rent, though this varies by agent and property.',
    category: 'renters',
    tags: ['income', 'payslips', 'documents', 'self-employed', 'centrelink', 'application'],
  },
  {
    id: 'renter-lease-types',
    question: 'What is the difference between a fixed-term and periodic lease?',
    answer: 'A fixed-term lease runs for a set period — usually 6 or 12 months — and gives both you and the landlord certainty. During this period, the landlord cannot increase the rent or ask you to vacate without grounds. A periodic (month-to-month) lease continues after a fixed term ends and offers more flexibility to leave with relatively short notice. The trade-off is that the landlord can also give you notice to vacate with the required notice period for your state. Most agents prefer fixed-term agreements initially, switching to periodic at renewal if both parties are happy.',
    category: 'renters',
    tags: ['fixed term', 'periodic', 'lease', 'month to month', 'notice'],
  },

  // VENDORS — EXPANDED
  {
    id: 'vendor-private-treaty',
    question: 'What is private treaty sale and how does it work?',
    answer: 'Private treaty is the most common sale method in Australia — the property is listed at an asking price and buyers negotiate directly with your agent until a price is agreed. Unlike auction, private treaty includes a cooling-off period in most states (typically 3–5 business days), giving the buyer a right to withdraw after signing the contract. Your agent will recommend private treaty or auction based on your property type, price range, and current market conditions in your suburb.',
    category: 'vendors',
    tags: ['private treaty', 'sale method', 'negotiation', 'cooling off'],
  },
  {
    id: 'vendor-eoi',
    question: 'What is an Expression of Interest (EOI) sale?',
    answer: 'An Expression of Interest sale means no asking price is advertised — buyers are invited to submit their best written offer by a set closing date. EOI is common for premium properties, properties with unusual features that make pricing difficult, or markets where the agent wants to test buyer appetite without committing to a price. Your agent reviews all submitted offers after the deadline and presents them to you. You are not obligated to accept any offer. Cooling-off typically applies to EOI contracts (confirm with your solicitor for your state).',
    category: 'vendors',
    tags: ['eoi', 'expression of interest', 'sale method', 'offers', 'no price'],
  },
  {
    id: 'vendor-after-offer',
    question: 'What happens after I receive an offer?',
    answer: 'Your agent will present the offer — price, conditions (subject to finance, subject to building inspection), proposed settlement date, and deposit details. You can accept, reject, or counter-offer. Once both parties sign the contract, the cooling-off period begins for private treaty sales. After cooling-off expires, the contract is unconditional — the buyer is legally committed. Unconditional contracts proceed to settlement on the agreed date.',
    category: 'vendors',
    tags: ['offer', 'accept', 'counter offer', 'contract', 'cooling off', 'unconditional'],
  },
  {
    id: 'vendor-settlement',
    question: 'What happens at settlement?',
    answer: 'Settlement is the legal transfer of ownership from you to the buyer, usually 30–90 days after contracts are exchanged. Your solicitor or conveyancer attends settlement on your behalf — most settlements in Australia are now conducted electronically via PEXA. The buyer\'s funds arrive in your nominated account, your mortgage is discharged, and the buyer receives the keys. After settlement you receive the net proceeds minus outstanding mortgage, agent commission, and legal fees.',
    category: 'vendors',
    tags: ['settlement', 'pexa', 'conveyancer', 'keys', 'transfer', 'proceeds'],
  },
  {
    id: 'vendor-prepare-sale',
    question: 'How should I prepare my property for sale?',
    answer: 'The highest-return preparation tasks are: a deep clean and declutter, fresh paint in neutral colours, garden tidying, and minor repairs (loose handles, dripping taps, cracked tiles). Professional photography is arranged by your agent and included in most marketing campaigns. Your agent will advise whether a styling or staging service is worth the investment for your price range — it typically adds value above $600k in most markets. Your solicitor prepares the legal documents (Section 32 or Contract of Sale) separately — allow 1–2 weeks before your listing goes live.',
    category: 'vendors',
    tags: ['prepare', 'presentation', 'styling', 'photography', 'repairs', 'marketing'],
  },
  {
    id: 'vendor-agent-fees',
    question: 'How do agent commissions and fees work in Australia?',
    answer: 'Agent commission in Australia is negotiated — there is no fixed rate. Typical commission ranges from 1.5% to 3.5% of the sale price, with higher rates common in regional areas and lower rates in high-volume city markets. Commission is only paid on a successful sale — you pay nothing if the property does not sell, except agreed marketing costs (photography, online listing fees), typically $1,500–$5,000 depending on your suburb and campaign. Always confirm the commission rate, marketing budget, and what is included in writing before signing the agency agreement.',
    category: 'vendors',
    tags: ['commission', 'fees', 'agent fees', 'marketing costs', 'agency agreement'],
  },

  // PROPERTY MANAGEMENT — COMPLIANCE
  {
    id: 'pm-smoke-alarms',
    question: 'How do I track smoke alarm compliance across my portfolio?',
    answer: 'Dashboard → Compliance → Smoke Alarms shows every property in your rent roll with its current compliance status: Compliant (green), Requires Attention (amber), or Non-Compliant (red). For each property you can record the last service date, next service due date, and certificate number. Properties are sorted with overdue items first so you can action the most urgent gaps immediately. In Queensland, a compliance certificate from a licensed electrician is required before new tenancies begin — the platform flags QLD properties without a valid certificate.',
    category: 'property-managers',
    tags: ['smoke alarms', 'compliance', 'QLD', 'certificate', 'portfolio'],
  },
  {
    id: 'pm-bond-claims',
    question: 'How do I lodge and manage a bond claim?',
    answer: 'Dashboard → Compliance → Bond Claims lets you create and track bond claims at the end of a tenancy. Create a claim by selecting the tenancy, entering the bond amount held, and itemising deductions by category (cleaning, damage, rent arrears, missing items). Once finalised, lodge the claim with the bond authority in the tenant\'s state and record the authority reference number in the platform. The claim status progresses from Draft → Submitted → Approved, Partial, or Disputed as the process unfolds. All claim records and itemised deductions are exportable as a PDF evidence package for tribunal.',
    category: 'property-managers',
    tags: ['bond claim', 'bond', 'deductions', 'tribunal', 'authority', 'export'],
  },
  {
    id: 'pm-bond-authority',
    question: 'Which authority handles bond disputes in my state?',
    answer: 'Bond disputes are handled by the relevant state authority: NSW — NSW Fair Trading; VIC — Residential Tenancies Bond Authority (RTBA); QLD — Residential Tenancies Authority (RTA); WA — Consumer Protection WA; SA — Consumer and Business Services (CBS); ACT — ACT Civil and Administrative Tribunal (ACAT); TAS — Consumer, Building and Occupational Services; NT — NT Consumer Affairs. Filing deadlines matter: NSW allows 6 months from vacancy; QLD and SA allow 3 months. Always bring the signed entry and exit condition reports, repair invoices, and rent ledger to the hearing.',
    category: 'property-managers',
    tags: ['bond', 'dispute', 'tribunal', 'authority', 'state', 'RTBA', 'RTA', 'NCAT', 'VCAT'],
  },
  {
    id: 'pm-rent-increase',
    question: 'How do I process a rent increase legally?',
    answer: 'Dashboard → Rent Increases shows every tenancy with its last increase date, months elapsed, and whether it is eligible for a new increase under state law. All Australian states except the Northern Territory require a minimum 12-month interval between increases (NT requires 6 months). To process an increase: select the tenancy, enter the new weekly rent and proposed effective date. The platform calculates the earliest legal effective date and blocks you from setting a date that violates your state\'s notice requirements. Once confirmed, a formal rent increase notice is generated and emailed to the tenant.',
    category: 'property-managers',
    tags: ['rent increase', 'notice', 'state law', '12 months', 'interval', 'compliance'],
  },
  {
    id: 'pm-rent-increase-notice',
    question: 'How much notice do I need to give for a rent increase?',
    answer: 'Notice requirements by state: NSW — 60 days; VIC — 60 days; QLD — 60 days; WA — 60 days; SA — 60 days; ACT — 56 days; TAS — 60 days; NT — 30 days. The ListHQ Rent Increases page enforces these minimums automatically — the earliest effective date it allows is always at least the required notice period from today. The notice email sent to the tenant constitutes formal written notice as required by law. Keep a copy of the sent notice in the tenancy record in case of a dispute.',
    category: 'property-managers',
    tags: ['rent increase', 'notice period', 'days', 'state law', '60 days', 'NT', 'ACT'],
  },
  {
    id: 'pm-bank-reconciliation',
    question: 'How does bank reconciliation work in trust accounting?',
    answer: 'Dashboard → Trust → Bank Reconciliation matches your bank statement transactions against your recorded trust receipts and payments. Add transactions from your bank statement manually or import them via CSV. Match each line to the corresponding receipt or payment in your trust ledger — matched items show in green, unmatched items stay in red. Use Auto-Match to let the platform suggest matches based on amount and date. The reconciliation is complete when your bank balance equals your ledger balance — click Reconcile All to lock the period. Monthly trust account reconciliation is a legal requirement in all Australian states and a standard audit finding if missed.',
    category: 'property-managers',
    tags: ['bank reconciliation', 'trust accounting', 'reconcile', 'audit', 'compliance', 'CSV'],
  },

  // AGENT WORKFLOWS
  {
    id: 'agent-voice-leads',
    question: 'What are Voice Leads and how do I use them?',
    answer: 'Voice Leads (Dashboard → Voice Leads) is your feed of buyers who used ListHQ\'s voice search feature. When a buyer speaks a property search query, the platform transcribes it, extracts their requirements, and — if it matches your listings or area — surfaces the lead in your feed. Each lead shows: the buyer\'s transcribed query, an urgency rating (Hot, Warm, or Cold) based on their readiness score, pre-approval status, preferred contact method, and any keywords extracted (pool, garage, north-facing). Hot leads score 70+ and are typically ready to purchase within weeks. Click Save to CRM to add any lead to your contacts.',
    category: 'agents',
    tags: ['voice leads', 'AI', 'readiness score', 'hot leads', 'CRM'],
  },
  {
    id: 'agent-pipeline',
    question: 'How does the Buyer Pipeline work?',
    answer: 'The Buyer Pipeline (Dashboard → Buyer Pipeline) is a kanban board that tracks every active buyer deal through your sales process. Deals move through five stages: Prospecting, Appraisal, Listed, Under Offer, and Settled. Each card shows the property address, buyer name, estimated deal value, and days in the current stage. Drag a card to move it to the next stage — the listing status updates automatically. From any card you can log an offer (price, conditions, expiry), record the outcome, or open a settlement checklist. Your principal or admin can see a real-time view of your entire book of business across the pipeline.',
    category: 'agents',
    tags: ['pipeline', 'kanban', 'buyer pipeline', 'deal tracking', 'stages', 'offer'],
  },
  {
    id: 'agent-settlement-concierge',
    question: 'What is Settlement Concierge?',
    answer: 'Settlement Concierge (Dashboard → Settlements) is your post-sale checklist tool for deals in the Under Offer and Settled stages. For each settlement it tracks five key actions: final inspection booked, keys handover arranged, trust funds cleared, buyer notified of settlement time, and Google review requested. A one-click panel of utility connection partners — AGL, Origin Energy, Telstra, NBN, and conveyancing — lets you refer your buyer to set up services immediately after settlement, adding genuine value and increasing the likelihood of a positive review.',
    category: 'agents',
    tags: ['settlement', 'concierge', 'checklist', 'utilities', 'keys', 'post-sale'],
  },
];

export const FAQ_CATEGORIES = [
  { key: 'general', label: 'General', icon: 'HelpCircle' },
  { key: 'agents', label: 'Agents', icon: 'Building2' },
  { key: 'property-managers', label: 'Property Managers', icon: 'ClipboardList' },
  { key: 'buyers', label: 'Buyers', icon: 'Home' },
  { key: 'renters', label: 'Renters', icon: 'Key' },
  { key: 'vendors', label: 'Vendors', icon: 'BarChart3' },
  { key: 'auctions', label: 'Auctions', icon: 'Hammer' },
  { key: 'billing', label: 'Billing', icon: 'CreditCard' },
  { key: 'technical', label: 'Technical', icon: 'Settings' },
  { key: 'halo', label: 'Halo', icon: 'Zap' },
] as const;
