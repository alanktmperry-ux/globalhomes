  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        (supabase.from('support_tickets').select('id', { count: 'exact', head: true }).eq('status', 'open')) as any,
        supabase.from('agents').select('id', { count: 'exact', head: true }).eq('is_subscribed', false).lt('created_at', monthStart),
        (supabase.from('buyer_activity_events').select('id', { count: 'exact', head: true }).eq('event_type', 'search').gte('created_at', todayStart)) as any,
        (supabase.from('saved_properties').select('id', { count: 'exact', head: true }).gte('saved_at', todayStart)) as any,
        (supabase.from('buyer_profiles').select('id', { count: 'exact', head: true })) as any,
        (supabase.from('buyer_profiles').select('id', { count: 'exact', head: true }).gte('created_at', d7)) as any,
        (supabase.from('partners').select('id', { count: 'exact', head: true })) as any,
        (supabase.from('brokers').select('id', { count: 'exact', head: true })) as any,
        (supabase.from('referral_leads').select('id', { count: 'exact', head: true }).gte('created_at', monthStart)) as any,
        (supabase.from('referral_leads').select('platform_fee_amount').eq('status', 'settled').gte('settled_at', monthStart)) as any,
      ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <KPI
          ...
          value={
            data.mrrGrowthPct != null
              ? `${data.mrrGrowthPct > 0 ? '+' : ''}${data.mrrGrowthPct}%`
              : data.paidAgents > 0 ? 'New' : '—'
          }
