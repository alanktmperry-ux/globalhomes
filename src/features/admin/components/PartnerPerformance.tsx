  ...
  const loadAgencies = async () => {
    ...
    try {
      ...
      const [agentsRes, balancesRes] = await Promise.all([
        ...
        supabase.from('trust_account_balances_view').select('agent_id, current_balance, last_reconciled_date'),
      ]);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const balancesRes = agentIds.length > 0
        ? await supabase.from('trust_account_balances_view').select('agent_id, current_balance, last_reconciled_date').in('agent_id', agentIds)
        : { data: [] as { agent_id: string; current_balance: number; last_reconciled_date: string | null }[] };
