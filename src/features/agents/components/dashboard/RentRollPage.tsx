...
import { Plus, Home, Loader2, AlertTriangle, CheckCircle2, Clock, ClipboardCheck, ChevronDown, ChevronUp, Calendar, RefreshCw, Wallet, CalendarPlus } from 'lucide-react';
...
const RentRollPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (form.tenant_email) {
      const { data: contactMatch } = await (supabase.from('contacts') as any)
        .select('id')
        .eq('agent_id', agentId)
