...
export default function TrustStatementModal({ open, onOpenChange }: TrustStatementModalProps) {
  ...
  const getComplianceLegislation = (state?: string | null): string => {
    switch ((state || '').toUpperCase()) {
      case 'NSW': return 'Property and Stock Agents Act 2002 (NSW) and Property and Stock Agents Regulation 2022 (NSW)';
      case 'VIC': return 'Estate Agents Act 1980 (Vic) and Residential Tenancies Act 1997 (Vic)';
      case 'QLD': return 'Agents Financial Administration Act 2014 (Qld) and Property Occupations Act 2014 (Qld)';
      case 'WA': return 'Real Estate and Business Agents Act 1978 (WA)';
      case 'SA': return 'Land Agents Act 1994 (SA)';
      case 'TAS': return 'Property Agents and Land Transactions Act 2005 (Tas)';
      case 'ACT': return 'Agents Act 2003 (ACT)';
      case 'NT': return 'Agents Licensing Act 1979 (NT)';
      default: return 'Applicable state real estate trust accounting legislation';
    }
  };
  ...
  const handleGeneratePdf = () => {
    ...
    const complianceLegislation = getComplianceLegislation(agentInfo?.state);
...
<div class="statement">
  ...
  <!-- Footer -->
  <div class="footer">
    <p><strong>Compliance Notice:</strong> This statement is prepared in accordance with the ${complianceLegislation}.</p>
    ...
    try {
      ...
      const transactions = [
        ...receipts.map(r => ({
          ...
          description: `${r.receipt_number} — ${r.client_name}${r.property_address ? ` (${r.property_address})` : ''}`,
          type: 'receipt' as const,
          amount: r.amount,
        })),
        ...payments.map(p => ({
          date: p.date_paid,
          description: `${p.payment_number} — ${p.payee_name || p.client_name}${p.property_address ? ` (${p.property_address})` : ''}`,
      ...
      await supabase.functions.invoke('send-notification-email', {
        body: {
          ...
          agency_name: agentInfo.agency,
          property_address: agentInfo.agency || 'Managed Portfolio',
          ...
          transactions,
          agent_license: agentInfo.license_number || 'N/A',
          agent_state_legislation: getComplianceLegislation(agentInfo.state),
        },
      });
          ...
          <>
            ...
            <div className="flex items-start gap-2 bg-muted/40 rounded-lg p-3 border border-border">
              ...
              <div className="text-[10px] text-muted-foreground leading-relaxed">
                ...
                <p>
                  Statement complies with the <strong>{getComplianceLegislation(agentInfo?.state)}</strong>.
                  Includes GST summary, sequential receipt/payment numbers, and signature lines. Retain for minimum 5 years.
                </p>
              </div>
            </div>
          </>
