...
export default function AgencyOnboardingPage() {
  ...
  const completeOnboarding = async () => {
    if (!user) return;
    const { error } = await supabase.from('agents').update({ onboarding_complete: true, is_approved: true } as any).eq('user_id', user.id);
    if (error) throw error;
    await refreshRoles();
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <AddressAutocomplete
                  ...
                  placeholder={isAustralia ? '123 High Street, Richmond VIC 3121' : 'Office street address'}
                  className="mt-1.5"
                />
                {agencyAddress && (
                  <p className="text-[11px] text-muted-foreground mt-1">Pre-filled from your registration — edit if needed</p>
