            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {agent.investmentNiche && (
                  <>
                    {agent.investmentNiche.split(',').map((s: string) => s.trim()).filter(Boolean).map((s: string) => (
                      <span key={s} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium border border-primary/20">
                        {s}
                      </span>
                    ))}
                  </>
