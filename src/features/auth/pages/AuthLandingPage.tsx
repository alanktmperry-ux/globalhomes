      ...
      >
        ...
        <div className="relative z-10 max-w-sm">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-slate-200 bg-slate-50 mb-6">
            ...
            <span className="text-[10px] font-medium tracking-widest uppercase text-slate-500">{t('auth.landing.buyerEyebrow')}</span>
          </div>

          <h1 className="text-[42px] font-light text-slate-900 leading-[1.05] mb-2" style={{ letterSpacing: '-2px' }}>
            {t('auth.landing.buyerHeadline1')}<br /><strong className="font-semibold">{t('auth.landing.buyerHeadline2')}</strong>
          </h1>
          <p className="text-[13px] text-slate-400 mb-6">{t('auth.landing.buyerSub')}</p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-8">
            {buyerChips.map((chip) => (
              ...
              >
                ...
                <div className="text-[10px] text-slate-400 leading-snug">{t(chip.descKey)}</div>
              </div>
            ))}
          </div>
      ...
      >
        ...
        <div className="relative z-10 max-w-sm">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-6" style={{ border: '1px solid rgba(59,91,219,0.4)', background: 'rgba(59,91,219,0.2)' }}>
            ...
            <span className="text-[10px] font-medium tracking-widest uppercase text-blue-400">{t('auth.landing.agentEyebrow')}</span>
          </div>

          <h1 className="text-[42px] font-light text-white leading-[1.05] mb-2" style={{ letterSpacing: '-2px' }}>
            {t('auth.landing.agentHeadline1')}<br />{t('auth.landing.agentHeadline2')} <strong className="font-semibold" style={{ color: '#60a5fa' }}>{t('auth.landing.agentHeadlineEm')}</strong>
          </h1>
          <p className="text-[13px] mb-6" style={{ color: 'rgba(255,255,255,0.35)' }}>{t('auth.landing.agentSub')}</p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-8">
            {agentChips.map((chip) => (
              ...
              >
                ...
                <div className="text-[10px] leading-snug" style={{ color: '#64748b' }}>{t(chip.descKey)}</div>
              </div>
            ))}
          </div>
