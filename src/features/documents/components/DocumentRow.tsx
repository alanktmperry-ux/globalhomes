...
function mimeLabel(mime?: string): { label: string; className: string } {
  ...
  if (mime.includes('pdf')) return { label: 'PDF', className: 'bg-red-50 text-red-700' };
  if (mime.includes('word') || mime.includes('document')) return { label: 'WORD', className: 'bg-blue-50 text-blue-700' };
  if (mime.includes('sheet') || mime.includes('excel')) return { label: 'EXCEL', className: 'bg-emerald-50 text-emerald-700' };
  if (mime.includes('image')) return { label: 'IMAGE', className: 'bg-purple-50 text-purple-700' };
  if (mime.includes('zip')) return { label: 'ZIP', className: 'bg-amber-50 text-amber-700' };
      ...
      <div className="flex items-start gap-2.5 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {doc.signed && (
              <Badge className="text-[9px] bg-emerald-500/15 text-emerald-700 border-0 gap-0.5">
                ✓ Signed
              </Badge>
            )}
          </div>
