  ...
  private parseManusOutput(output: any): Property[] {
    try {
      ...
      return properties.map((p: any, i: number) => ({
        ...
        priceFormatted: p.priceFormatted || p.price_formatted || new Intl.NumberFormat(params.language || 'en-AU', { style: 'currency', currency: 'AUD', maximumFractionDigits: 0 }).format(p.price || 0),
