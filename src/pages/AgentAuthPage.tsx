...
import { Mail, Lock, User, ArrowLeft, Building2, Phone, KeyRound, Plus } from 'lucide-react';
...
type SignupPath = 'choose' | 'create-agency' | 'join-agency';

const AgentAuthPage = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const [mode, setMode] = useState<AuthMode>('login');
  const [signupPath, setSignupPath] = useState<SignupPath>('choose');
    ...
    try {
      if (mode === 'signup') {
        ...
        if (signupPath === 'create-agency') {
          ...
          // Create agent record linked to agency
          ...
          // Increment uses
          await supabase.from('agency_invite_codes')
  ...
  const inputClass = "w-full pl-10 pr-4 py-3 rounded-xl bg-secondary text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30";
      ...
      <main className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full px-6 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="font-display text-2xl font-bold text-foreground mb-1">
            {mode === 'login' ? 'Agent Sign In' : signupPath === 'create-agency' ? 'Create Your Agency' : signupPath === 'join-agency' ? 'Join an Agency' : 'Get Started'}
          </h1>
          <p className="text-sm text-muted-foreground mb-6">
            {mode === 'login'
              ? 'Access your dashboard and manage listings'
              : signupPath === 'choose'
              ? 'Create a new agency or join an existing one'
              : signupPath === 'create-agency'
              ? 'Set up your company profile and start adding staff'
          ...
          {/* Signup path chooser */}
          {mode === 'signup' && signupPath === 'choose' && (
            <div className="space-y-3 mb-6">
              <button
                onClick={() => setSignupPath('create-agency')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl bg-card border border-border text-left hover:border-primary/50 transition-colors"
              >
              ...
              <button
                onClick={() => setSignupPath('join-agency')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl bg-card border border-border text-left hover:border-primary/50 transition-colors"
              >
            ...
            <>
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm transition-transform active:scale-[0.98] disabled:opacity-50">
                  {loading ? 'Please wait...' : signupPath === 'create-agency' ? 'Create Agency & Account' : 'Join Agency'}
                </button>
              </form>

              <button
                onClick={() => setSignupPath('choose')}
          ...
          {mode === 'login' && (
            <>
              ...
              <p className="text-center text-sm text-muted-foreground mt-6">
                ...
                <button onClick={() => { setMode('signup'); setSignupPath('choose'); }} className="text-primary font-medium">Register</button>
              </p>
