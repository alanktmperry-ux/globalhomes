...
interface RoutineHistoryItem {
  inspection: { id: string; conducted_date: string | null; overall_notes: string | null };
  rooms: { id: string; room_name: string; condition: string | null; notes: string | null }[];
}
...
const InspectionReportPage = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (inspData.inspection_type === 'exit') {
      ...
      // Fetch all completed routine inspections for this tenancy (for history panel)
      const { data: routineInsps } = await supabase
        .from('property_inspections')
        .select('id, conducted_date, overall_notes')
        .eq('tenancy_id', inspData.tenancy_id)
        .eq('inspection_type', 'routine')
        .eq('status', 'completed')
        .order('conducted_date', { ascending: false });

      if (routineInsps && routineInsps.length > 0) {
        const histItems = await Promise.all(
          (routineInsps as { id: string; conducted_date: string | null; overall_notes: string | null }[]).map(async (ri) => {
            const { data: riRooms } = await supabase
              .from('inspection_rooms')
              .select('id, room_name, condition, notes')
              .eq('inspection_id', ri.id)
              .order('display_order');
            return {
              inspection: ri,
              rooms: (riRooms || []) as { id: string; room_name: string; condition: string | null; notes: string | null }[],
            };
          })
        );
        setRoutineHistory(histItems);
      } else {
        setRoutineHistory([]);
      }
    }
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      {/* Routine Inspection History — only for exit inspections */}
      {inspection.inspection_type === 'exit' && routineHistory.length > 0 && (
            ...
            <CollapsibleTrigger asChild>
              <Button variant="outline" className="w-full justify-between">
                <span className="flex items-center gap-2">
                  ...
                  Routine Inspection History ({routineHistory.length})
                </span>
                <ChevronDown size={14} />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent>
              <div className="space-y-3 mt-3">
                {routineHistory.map(h => (
                  <Card key={h.inspection.id}>
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold">
                          {h.inspection.conducted_date
                            ? format(parseISO(h.inspection.conducted_date), 'dd MMM yyyy')
                            : 'Date not recorded'}
                        </p>
                        <Badge className="text-[10px] bg-teal-500/15 text-teal-700 border-0">Routine</Badge>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                        {h.rooms.map(r => (
                          <div key={r.id} className="flex items-center justify-between gap-1 text-xs">
                            <span className="text-muted-foreground truncate">{r.room_name}</span>
                            {r.condition && (
                              <Badge className={cn('border capitalize text-[10px] shrink-0', CONDITION_COLORS[r.condition] || '')}>
                                {r.condition}
                              </Badge>
                            )}
                          </div>
                      ...
                      {h.inspection.overall_notes && (
                        <p className="text-xs text-muted-foreground border-t pt-2">{h.inspection.overall_notes}</p>
                      )}
                    </CardContent>
                  </Card>
                ))}
