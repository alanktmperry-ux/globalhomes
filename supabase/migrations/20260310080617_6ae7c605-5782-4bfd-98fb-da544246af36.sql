...
-- Recreate function with nested exception handling so notification insert isn't rolled back
...
BEGIN
  ...
  -- Try sending email in a nested block so notification insert is preserved
  BEGIN
...
BEGIN
  ...
  -- Try sending email in a nested block
  BEGIN
    PERFORM net.http_post(
      url := current_setting('app.settings.supabase_url', true) || '/functions/v1/send-notification-email',
      headers := jsonb_build_object(
