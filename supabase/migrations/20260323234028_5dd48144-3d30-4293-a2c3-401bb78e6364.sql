...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Approved reviews are public' AND tablename = 'agent_reviews') THEN
    CREATE POLICY "Approved reviews are public" ON public.agent_reviews FOR SELECT TO anon, authenticated USING (status = 'approved');
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Agents see own reviews' AND tablename = 'agent_reviews') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Anyone can submit review' AND tablename = 'agent_reviews') THEN
    CREATE POLICY "Anyone can submit review" ON public.agent_reviews FOR INSERT TO anon, authenticated WITH CHECK (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Agents manage own requests' AND tablename = 'review_requests') THEN
...
CREATE OR REPLACE FUNCTION public.refresh_agent_rating(p_agent_id uuid) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.on_review_status_change() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS review_status_change ON public.agent_reviews;
