...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Anyone can submit ticket' AND tablename = 'support_tickets') THEN
    CREATE POLICY "Anyone can submit ticket" ON public.support_tickets FOR INSERT TO anon, authenticated WITH CHECK (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admins see all tickets' AND tablename = 'support_tickets') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Feature requests readable' AND tablename = 'feature_requests') THEN
    CREATE POLICY "Feature requests readable" ON public.feature_requests FOR SELECT TO authenticated USING (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Agents submit feature requests' AND tablename = 'feature_requests') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Admins manage feature requests' AND tablename = 'feature_requests') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Agents upvote features' AND tablename = 'feature_request_upvotes') THEN
