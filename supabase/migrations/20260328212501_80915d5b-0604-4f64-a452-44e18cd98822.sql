CREATE OR REPLACE FUNCTION delete_user_cascade(p_user_id UUID)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_agent_ids UUID[];
  v_agency_ids UUID[];
  v_prop_ids UUID[];
  v_trust_account_ids UUID[];
  v_trust_tx_ids UUID[];
  v_conv_ids UUID[];
BEGIN
  SELECT ARRAY_AGG(id) INTO v_agent_ids FROM agents WHERE user_id = p_user_id;
  v_agent_ids := COALESCE(v_agent_ids, '{}');

  SELECT ARRAY_AGG(id) INTO v_agency_ids FROM agencies WHERE owner_user_id = p_user_id;
  v_agency_ids := COALESCE(v_agency_ids, '{}');

  IF array_length(v_agent_ids, 1) > 0 THEN
    SELECT ARRAY_AGG(id) INTO v_prop_ids FROM properties WHERE agent_id = ANY(v_agent_ids);
  END IF;
  v_prop_ids := COALESCE(v_prop_ids, '{}');

  IF array_length(v_agent_ids, 1) > 0 THEN
    SELECT ARRAY_AGG(id) INTO v_trust_account_ids FROM trust_accounts WHERE agent_id = ANY(v_agent_ids);
  END IF;
  v_trust_account_ids := COALESCE(v_trust_account_ids, '{}');

  IF array_length(v_trust_account_ids, 1) > 0 THEN
    SELECT ARRAY_AGG(id) INTO v_trust_tx_ids FROM trust_transactions WHERE trust_account_id = ANY(v_trust_account_ids);
  END IF;
  v_trust_tx_ids := COALESCE(v_trust_tx_ids, '{}');

  SELECT ARRAY_AGG(id) INTO v_conv_ids FROM conversations
    WHERE participant_1 = p_user_id OR participant_2 = p_user_id;
  v_conv_ids := COALESCE(v_conv_ids, '{}');

  IF array_length(v_prop_ids, 1) > 0 THEN
    DELETE FROM listing_documents WHERE property_id = ANY(v_prop_ids);
    DELETE FROM saved_properties WHERE property_id = ANY(v_prop_ids);
    DELETE FROM lead_events WHERE property_id = ANY(v_prop_ids);
    DELETE FROM leads WHERE property_id = ANY(v_prop_ids);
    DELETE FROM notifications WHERE property_id = ANY(v_prop_ids);
    DELETE FROM collab_reactions WHERE property_id = ANY(v_prop_ids);
    DELETE FROM collab_views WHERE property_id = ANY(v_prop_ids);
    DELETE FROM off_market_shares WHERE property_id = ANY(v_prop_ids);
    DELETE FROM rental_applications WHERE property_id = ANY(v_prop_ids);
    DELETE FROM transactions WHERE property_id = ANY(v_prop_ids);
    DELETE FROM trust_transactions WHERE property_id = ANY(v_prop_ids);
  END IF;

  IF array_length(v_trust_tx_ids, 1) > 0 THEN
    DELETE FROM off_market_shares WHERE trust_entry_id = ANY(v_trust_tx_ids);
  END IF;

  IF array_length(v_trust_account_ids, 1) > 0 THEN
    DELETE FROM trust_transactions WHERE trust_account_id = ANY(v_trust_account_ids);
    DELETE FROM trust_accounts WHERE id = ANY(v_trust_account_ids);
  END IF;

  IF array_length(v_agent_ids, 1) > 0 THEN
    DELETE FROM trust_account_balances WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM trust_receipts WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM trust_payments WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM trust_reconciliations WHERE agent_id = ANY(v_agent_ids);
  END IF;

  IF array_length(v_agent_ids, 1) > 0 THEN
    DELETE FROM notifications WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM lead_events WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM leads WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM agent_subscriptions WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM agent_credentials WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM agent_locations WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM off_market_shares WHERE sharing_agent_id = ANY(v_agent_ids);
    DELETE FROM off_market_shares WHERE shared_with_agent_id = ANY(v_agent_ids);
    DELETE FROM contacts WHERE assigned_agent_id = ANY(v_agent_ids);
    DELETE FROM rental_applications WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM transactions WHERE agent_id = ANY(v_agent_ids);
    DELETE FROM properties WHERE id = ANY(v_prop_ids);
    DELETE FROM agents WHERE id = ANY(v_agent_ids);
  END IF;

  IF array_length(v_agency_ids, 1) > 0 THEN
    DELETE FROM agency_invite_codes WHERE agency_id = ANY(v_agency_ids);
    DELETE FROM agency_members WHERE agency_id = ANY(v_agency_ids);
    DELETE FROM activities WHERE office_id = ANY(v_agency_ids);
    DELETE FROM tasks WHERE office_id = ANY(v_agency_ids);
    DELETE FROM transactions WHERE office_id = ANY(v_agency_ids);
    DELETE FROM contacts WHERE agency_id = ANY(v_agency_ids);
    UPDATE agents SET agency_id = NULL WHERE agency_id = ANY(v_agency_ids);
    DELETE FROM agencies WHERE id = ANY(v_agency_ids);
  END IF;

  IF array_length(v_conv_ids, 1) > 0 THEN
    DELETE FROM messages WHERE conversation_id = ANY(v_conv_ids);
  END IF;
  DELETE FROM messages WHERE sender_id = p_user_id;
  DELETE FROM conversations WHERE participant_1 = p_user_id OR participant_2 = p_user_id;

  DELETE FROM agency_members WHERE user_id = p_user_id;
  DELETE FROM agency_invite_codes WHERE created_by = p_user_id;
  DELETE FROM saved_properties WHERE user_id = p_user_id;
  DELETE FROM saved_search_alerts WHERE user_id = p_user_id;
  DELETE FROM buyer_profiles WHERE user_id = p_user_id;
  DELETE FROM collab_reactions WHERE user_id = p_user_id;
  DELETE FROM collab_views WHERE user_id = p_user_id;
  DELETE FROM contact_activities WHERE user_id = p_user_id;
  DELETE FROM contacts WHERE created_by = p_user_id;
  DELETE FROM activities WHERE user_id = p_user_id;
  DELETE FROM tasks WHERE user_id = p_user_id;
  DELETE FROM leads WHERE user_id = p_user_id;
  DELETE FROM lead_events WHERE user_id = p_user_id;
  DELETE FROM user_preferences WHERE user_id = p_user_id;
  DELETE FROM user_roles WHERE user_id = p_user_id;
  DELETE FROM profiles WHERE user_id = p_user_id;
END;
$$;