DROP FUNCTION IF EXISTS public.compute_halo_matches(uuid);

CREATE FUNCTION public.compute_halo_matches(p_halo_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
    ...
    SELECT
      ...
      jsonb_strip_nulls(jsonb_build_array(
        ...
        CASE WHEN semantic_sim IS NOT NULL AND semantic_sim >= 0.55 THEN to_jsonb(
          format('Portfolio semantically matches (%s%%)', ROUND((semantic_sim * 100)::numeric))
        ) END
      )) AS reasons
    FROM scored
  )
